...
def artifact_entries() -> List[Dict[str, Any]]:
    ...
    for group, title, path in ARTIFACTS:
        p = find_existing_input(path)
        generated_this_run = path in COMMAND_CENTER_GENERATED_ARTIFACTS and not p.exists()
        exists = p.exists() or generated_this_run
        source_path = p.as_posix() if p.exists() else output_path(path).as_posix() if generated_this_run else ""
        snippet = ""
        ...
        entries.append(
            {
                ...
                "status_detail": "Ready to open" if p.exists() else "Created with this command center run" if generated_this_run else missing_artifact_detail(path),
