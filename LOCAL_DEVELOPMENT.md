...
The same command center pass also writes review-only render prep packets: `render_prep_packets.md`, `render_prep_packets.csv`, and `render_prep_packets.json`. These packets are created only from render-ready or render-prep story candidates that clear source, asset, format, and manual-path gates. They provide template fit, copy fields, asset requirements, and exact manual renderer steps, but they do not render files, auto-run graphics, call paid APIs, or publish. The top packet also gets a review-only handoff folder at `render_handoff_top_packet/` with `README.md`, `copy_sheet.md/.csv`, `asset_checklist.md/.csv`, `active_asset_review_queue.md/.csv`, `source_proof.md`, `manual_renderer_prompt.md`, and `handoff_manifest.json`.

To create local review drafts from that handoff, run `.\hsd.cmd run -Mode render` manually after a review run. This mode reads `render_handoff_top_packet/` from the current run/root/latest local output, copies the handoff into a new run folder, and writes a template-driven primary preview at `render_handoff_top_packet/draft_preview.png` plus format-specific review drafts at `render_handoff_top_packet/review_drafts/draft_preview_ig_feed.png`, `render_handoff_top_packet/review_drafts/draft_preview_story.png`, and `render_handoff_top_packet/review_drafts/draft_preview_square.png`. It also writes `manual_review_renderer_report.md`, `manual_review_renderer_manifest.json`, `manual_visual_qa_report.md`, `manual_visual_qa_manifest.json`, `manual_visual_qa_checklist.csv`, `manual_visual_qa_approval_intake.md/.csv/.json`, `manual_visual_qa_operator_decision_draft.md/.csv/.json`, `manual_visual_qa_operator_decision_template.md/.csv/.json`, `manual_visual_qa_operator_decision_intake.md/.csv/.json`, `manual_post_approval_render_staging.md/.csv/.json`, and `manual_visual_qa_operator_decision_walkthrough.md/.csv/.json`. Render mode also refreshes `operator_command_center.html` with a Decision tab that shows the draft preview, QA status, approve/hold/revise controls, required notes, copy-safe CSV row output, and validation feedback. The preview, format drafts, visual QA report, approval intake, copy-safe operator decision draft, copy-only decision template, operator decision intake validation, staging guidance, walkthrough, and UI panel are drafts for human review only; they are not approved, not publish-ready, and not generated by an automatic workflow. To create the local operator decision inbox shell, run `.\hsd.cmd run -Mode decision-inbox`; that explicit mode creates only `operator/inbox/manual_visual_qa_operator_decisions.csv` with a header row if it does not already exist, writes `manual_visual_qa_operator_decision_inbox_starter.md/.csv/.json`, and never fills decisions, approves, moves files, publishes, or creates a publish-ready lane. Human decisions can be copied into `operator/inbox/manual_visual_qa_operator_decisions.csv`; render mode validates that inbox against the generated draft before staging reads it, and placeholder template values are rejected.
...
- `identity-decision`: localhost-only manual save mode for WNBA athlete identity-resolution rows from the Decision tab; writes only to `operator/inbox/wnba_athlete_identity_resolution.csv`.
- `identity-decision-verify`: localhost handler write-back verifier that posts a command-center-derived row, compares the saved CSV exactly, and restores the inbox.
