...
def test_manual_review_renderer_reads_latest_handoff_and_writes_review_draft(tmp_path: Path) -> None:
    ...
    assert manifest["content_module"]["hero_image_mode"] == "logo_score_fallback_no_person_image"
    ...
    assert manifest["render_background_style"] == "hsd_premium_sports_editorial_v28_open_score_rails"
    ...
    assert "photo_first_editorial_stage_depth" in manifest["render_background_cues"]
    assert "photo_first_score_type_grid_polish" in manifest["render_background_cues"]
    assert "photo_first_type_scale" in manifest["render_background_cues"]
    assert "photo_first_athlete_visual_cap" in manifest["render_background_cues"]
    assert "photo_first_editorial_score_rails" in manifest["render_background_cues"]
    assert "photo_first_subtle_logo_identifiers" in manifest["render_background_cues"]
    ...
    assert "photo_first_editorial_focal_corridor" in manifest["render_background_cues"]
    assert "photo_first_calm_background_zones" in manifest["render_background_cues"]
    ...
    assert "photo_first_blueprint_depth_layers" in manifest["render_background_cues"]
    assert "photo_first_procedural_court_grain" in manifest["render_background_cues"]
    assert "photo_first_asymmetric_score_treatment" in manifest["render_background_cues"]
    ...
    assert "photo_first_open_lower_caption_rail" in manifest["render_background_cues"]
    ...
    assert all(item["render_background_style"] == "hsd_premium_sports_editorial_v28_open_score_rails" for item in manifest["format_options"])
    ...
    assert visual_board["background_style"] == "hsd_premium_sports_editorial_v28_open_score_rails"
    ...
    assert "Background style: `hsd_premium_sports_editorial_v28_open_score_rails`" in board
...
def test_manual_review_renderer_bridges_score_only_handoff_to_existing_stat_proof_athlete(tmp_path: Path, monkeypatch) -> None:
    ...
    public_copy = module.photo_first_public_canvas_copy(score, selected)
    assert public_copy["kicker"] == "WNBA FINAL"
    assert public_copy["result_line"] == "Sky beat Fire, 124-94"
    assert public_copy["athlete_line"] == "Kamilla Cardoso led Chicago with 30 points."
    assert public_copy["stat_line"] == "30 PTS / 8 REB / 1 AST"
    canvas_text = " ".join(public_copy.values()).upper()
    for banned_phrase in module.PUBLIC_RENDER_BANNED_CANVAS_PHRASES:
        assert banned_phrase not in canvas_text
    ...
    assert summary["hero_silhouette_mode"] in {"headshot_bridge_rectangular_source", "local_transparent_cutout_grid_breaking"}
    assert summary["hero_cutout_readiness"] in {"headshot_bridge_cutout_not_available", "cutout_ready_review_only_local_asset"}
...
def test_manual_review_renderer_selects_photo_layout_by_format() -> None:
    ...
    assert geometry["stat_strip_box"] == [60, 994, 960, 84]
...
def test_manual_review_renderer_photo_first_type_scale_and_athlete_cap_contract() -> None:
    import importlib.util
    ...
    assert len(module.PHOTO_FIRST_FINAL_SCORE_ACTIVE_TYPE_LEVELS) <= 4
    assert set(module.PHOTO_FIRST_FINAL_SCORE_ACTIVE_TYPE_LEVELS) == {"label", "headline", "score", "support"}
    for key in ["chip_label", "context_rail", "athlete_line", "stat", "review_marker"]:
    ...
    assert module.photo_first_type_spec("team", winner=True)["resolved_size"] >= 50
    assert module.photo_first_type_spec("team")["resolved_size"] <= 30
    assert module.photo_first_type_spec("score", winner=True)["resolved_size"] >= 120
    assert 52 <= module.photo_first_type_spec("score")["resolved_size"] <= 64
    assert module.PHOTO_FIRST_SCORE_ASYMMETRY_CONTRACT["loser_opacity"] <= 0.65
    ...
    for format_spec in [
        ...
        assert geometry["athlete_visual_max_share"] == module.PHOTO_FIRST_ATHLETE_MAX_VISUAL_SHARE
        assert geometry["athlete_visual_share"] <= module.PHOTO_FIRST_ATHLETE_MAX_VISUAL_SHARE
        assert geometry["athlete_visual_status"] == "athlete_supports_result"
...
def test_manual_review_renderer_blueprint_depth_cutout_and_safe_zone_contract(tmp_path: Path) -> None:
    import importlib.util
    ...
    opaque = tmp_path / "headshot.png"
    Image.new("RGB", (120, 160), (40, 80, 120)).save(opaque)
    transparent = tmp_path / "cutout.png"
    cutout = Image.new("RGBA", (120, 160), (40, 80, 120, 255))
    for x in range(0, 20):
        for y in range(0, 160):
            cutout.putpixel((x, y), (0, 0, 0, 0))
    cutout.save(transparent)

    headshot_contract = module.hero_cutout_mode_contract(
        {"athlete_photo_status": "approved_local_headshot", "athlete_photo_path": opaque.as_posix()}
    )
    cutout_contract = module.hero_cutout_mode_contract(
        {"athlete_photo_status": "approved_local_headshot", "athlete_photo_path": transparent.as_posix()}
    )
    assert headshot_contract["hero_silhouette_mode"] == "headshot_bridge_rectangular_source"
    assert headshot_contract["hero_cutout_readiness"] == "headshot_bridge_cutout_not_available"
    assert cutout_contract["hero_silhouette_mode"] == "local_transparent_cutout_grid_breaking"
    assert cutout_contract["hero_cutout_readiness"] == "cutout_ready_review_only_local_asset"
    assert "no downloads" in cutout_contract["grid_breaking_hero_contract"]

    geometry = module.photo_first_layout_geometry({"format_id": "ig_feed_4x5", "width": 1080, "height": 1350})
    assert geometry["safe_zone_px"] == {"top": 90, "bottom": 90, "left": 60, "right": 60}
    assert geometry["safe_zone_status"] == "critical_content_inside_safe_zone"
    assert geometry["hero_left_bleed_px"] > 0
    assert geometry["hero_grid_break_bleed_allowed"] is True
    assert geometry["team_color_weighting"] == "winner_palette_dominant_loser_palette_localized_subdued"
    ...
    winner_profile = {"accent_rgb": (190, 39, 54)}
    loser_profile = {"accent_rgb": (92, 70, 168)}
    module.draw_photo_first_blueprint_depth_layers(image, geometry, score, winner_profile, loser_profile, {}, {})
    diff = ImageChops.difference(before.convert("RGB"), image.convert("RGB"))
    assert diff.getbbox() is not None
    photo = geometry["photo_stage_box"]
    photo_diff = ImageChops.difference(
        before.crop((photo[0], photo[1], photo[0] + photo[2], photo[1] + photo[3])).convert("RGB"),
        image.crop((photo[0], photo[1], photo[0] + photo[2], photo[1] + photo[3])).convert("RGB"),
    )
    assert photo_diff.getbbox() is not None
...
def test_manual_review_renderer_photo_first_score_slab_stays_inside_score_row() -> None:
    ...
    for format_spec in [
        ...
        for key, winner in [("winner_score_row_box", True), ("loser_score_row_box", False)]:
            ...
            assert slab[2] >= 126
            assert slab[3] >= 70
            digit_cell = module.photo_first_score_digit_cell_box(slab)
            assert digit_cell[0] > slab[0]
            assert digit_cell[1] > slab[1]
            assert digit_cell[2] <= slab[0] + slab[2] - 10
            assert digit_cell[3] <= slab[1] + slab[3] - 10
...
def test_manual_review_renderer_photo_first_score_lock_slab_has_fitted_number_cell() -> None:
    ...
    assert "photo_first_score_type_grid_polish" in module.RENDER_BACKGROUND_CUES
    assert "photo_first_type_scale" in module.RENDER_BACKGROUND_CUES
...
def test_manual_review_renderer_logo_identifier_avoids_public_status_pill() -> None:
    ...
    result = module.draw_team_logo_slot(image, "Test Example", (40, 40, 100, 100), {}, {}, (190, 39, 54), winner=False)

    assert result["status"] == "logo_missing_team_name_placeholder"
    assert result["logo_review_required"] is True
    assert "photo_first_subtle_logo_identifiers" in module.RENDER_BACKGROUND_CUES
    cue_zone = image.crop((40, 100, 140, 140)).convert("RGB")
    data = cue_zone.tobytes()
    pixels = max(1, len(data) // 3)
    red_status_pixels = 0
    white_label_pixels = 0
    for index in range(0, len(data), 3):
        r, g, b = data[index], data[index + 1], data[index + 2]
        if r >= 135 and g <= 80 and b <= 100:
            red_status_pixels += 1
        if r >= 210 and g >= 210 and b >= 200:
            white_label_pixels += 1
    assert red_status_pixels / pixels < 0.12
...
def test_manual_review_renderer_photo_first_context_rail_has_score_hierarchy() -> None:
    ...
    assert "photo_first_context_stage_bridge" in module.RENDER_BACKGROUND_CUES
    x, y, w, h = box
    crop = image.crop((x, y, x + w, y + h)).convert("RGB")
    data = crop.tobytes()
    pixels = max(1, len(data) // 3)
    accent_bridge_pixels = 0
    gold_rule_pixels = 0
    light_text_pixels = 0
    dark_rail_pixels = 0
    for index in range(0, len(data), 3):
    ...
    assert accent_bridge_pixels / pixels > 0.001
...
def test_manual_review_renderer_stat_strip_draws_visible_proof_rail() -> None:
    ...
    for index in range(0, len(data), 3):
        ...
        if r >= 185 and g >= 145 and b <= 105:
            gold_pixels += 1
        if b >= 70 and 20 <= r <= 100 and 40 <= g <= 150 and b >= g:
            blue_pixels += 1
        if r <= 12 and g <= 15 and b <= 24:
            dense_panel_pixels += 1
...
def test_manual_review_renderer_background_draws_editorial_depth_markers_without_washing_title() -> None:
    ...
    for index in range(0, len(right_data), 3):
        ...
        if r > b and r >= 35 and g <= 45:
            red_bias_pixels += 1
    assert 0.010 < warm_pixels / (left_pixels + right_pixels) < 0.055
    assert blue_bias_pixels / left_pixels > 0.10
    assert 0.008 < red_bias_pixels / right_pixels < 0.12

    image = Image.new("RGBA", (1080, 1350), (2, 4, 9, 255))
    module.draw_reference_background(image, "final", (72, 144, 216), (192, 35, 48), photo_first=True)
    assert "photo_first_editorial_focal_corridor" in module.RENDER_BACKGROUND_CUES
    assert "photo_first_calm_background_zones" in module.RENDER_BACKGROUND_CUES
    ...
    score_corridor_crop = image.crop((420, 360, 1040, 890)).convert("RGB")
    corridor_data = score_corridor_crop.tobytes()
    corridor_pixels = max(1, len(corridor_data) // 3)
    calm_dark_pixels = 0
    hot_warm_pixels = 0
    for index in range(0, len(corridor_data), 3):
        r, g, b = corridor_data[index], corridor_data[index + 1], corridor_data[index + 2]
        if (r + g + b) / 3 <= 48:
            calm_dark_pixels += 1
        if r >= 86 and g >= 48 and b <= 35:
            hot_warm_pixels += 1
    assert calm_dark_pixels / corridor_pixels > 0.46
    assert hot_warm_pixels / corridor_pixels < 0.22
...
def test_manual_review_renderer_photo_first_depth_stage_adds_focal_atmosphere() -> None:
    ...
    assert "photo_first_score_stage_wash" in module.RENDER_BACKGROUND_CUES
    assert "photo_first_action_photo_stage_bridge" in module.RENDER_BACKGROUND_CUES
    athlete_crop = image.crop((40, 330, 500, 975)).convert("RGB")
    score_crop = image.crop((480, 360, 1040, 792)).convert("RGB")
    shelf_crop = image.crop((56, 952, 1024, 982)).convert("RGB")
...
def test_manual_review_renderer_photo_first_editorial_depth_bridge_ties_score_to_stat_band() -> None:
    ...
    assert "photo_first_open_lower_caption_rail" in module.RENDER_BACKGROUND_CUES
    ...
    assert gold_shelf_pixels / shelf_pixels > 0.007
...
def test_manual_review_renderer_square_lower_module_shows_body_line_without_heavy_card() -> None:
    import importlib.util
...
def test_manual_review_renderer_guardrail_uses_top_marker_and_footer_watermark_lock() -> None:
    ...
    marker_crop = image.crop((790, 72, 1034, 122)).convert("RGB")
    footer_crop = image.crop((54, 1010, 1026, 1063)).convert("RGB")
    data = marker_crop.tobytes()
    pixels = max(1, len(data) // 3)
    red_pixels = 0
    light_text_pixels = 0
    for index in range(0, len(data), 3):
...
def test_manual_review_renderer_photo_first_square_template_uses_diagnostic_watermark_lock() -> None:
    ...
    marker_zone = image.crop((790, 72, 1034, 122)).convert("RGB")
