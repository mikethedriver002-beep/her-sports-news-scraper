...
def test_local_runner_collects_game_source_confirmation_next_actions_for_latest() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    collector = runner[runner.index("function Collect-HsdArtifacts") : runner.index("function Invoke-HsdRun")]
...
def test_local_runner_collects_game_source_confirmation_return_summary_for_latest() -> None:
    runner = RUNNER.read_text(encoding="utf-8")
    collector = runner[runner.index("function Collect-HsdArtifacts") : runner.index("function Invoke-HsdRun")]
...
def test_local_runner_collects_final_score_stat_proof_for_latest() -> None:
    ...
    assert '"final_score_stat_proof_v1.md"' in collector
    assert '"final_score_stat_proof_v1.csv"' in collector
    assert '"final_score_stat_proof_v1.json"' in collector
...
def test_manual_render_mode_is_explicit_review_only() -> None:
    ...
    assert "render_next_level_editorial_qa.md" in runner
    assert "render_next_level_editorial_qa.csv" in runner
    assert "render_next_level_editorial_qa.json" in runner
    ...
    assert "identity_decision_live_writeback_verification.md" in runner
    assert "identity_decision_live_writeback_verification.json" in runner
