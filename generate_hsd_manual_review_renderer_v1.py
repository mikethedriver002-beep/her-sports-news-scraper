...
VERSION = "hsd-manual-review-renderer-v1.63.0-photo-first-open-rails"
...
def draw_photo_first_athlete_stage(
    image: Any,
    box: Tuple[int, int, int, int],
    module: Dict[str, Any],
    accent: tuple[int, int, int],
...
def draw_photo_first_stat_strip(
    image: Any,
    box: Tuple[int, int, int, int],
    module: Dict[str, Any],
    accent: tuple[int, int, int],
    canvas_copy: Dict[str, str] | None = None,
    *,
    format_id: str | None = None,
) -> None:
    x, y, w, h = box
    ...
    if clean(format_id) == "square_feed_1x1":
        wash_draw.polygon(
            [
                (x + 12, y + 24),
                (x + w - 18, y + 4),
    ...
    draw_reference_text(
        image,
        (x + 32, y + (18 if not compact else 14), w - 64, 40 if not compact else 30),
        athlete_line,
        athlete_type["font"],
    ...
    draw_reference_text(
        image,
        (x + 32, y + (58 if not compact else 42), w - 64, 32 if not compact else 24),
        stat_line,
        stat_type["font"],
...
def draw_photo_first_final_score_template(
    ...
    photo_ok = draw_photo_first_athlete_stage(image, photo_box, stat_module, winner_accent, focus_box, format_id=format_id)
    if not photo_ok:
        return False

    draw_photo_first_score_row(image, winner_box, score["winner"], score["winner_score"], winner_accent, aliases, logos, winner=True, format_id=format_id)
    draw_photo_first_score_row(image, loser_box, score["loser"], score["loser_score"], loser_accent, aliases, logos, winner=False, format_id=format_id)
    ...
    draw_photo_first_stat_strip(image, stat_box, stat_module, winner_accent, canvas_copy, format_id=format_id)
