HSD GRAPHICS PRODUCTION.

Use the uploaded ZIP only.

Create premium Her Sports Daily graphics for:
Tonight in the W

Variant: with_players
PLAYER MODE: WITH PLAYERS.
Use the uploaded approved player headshots if they improve the graphic. Do not invent player photos. Do not use generic blank headshots. If player_mode says team_candidate_players_review_required, these images are optional visual candidates, not locked statistical claims. Do not add player stats unless supplied in the dossier.

Detected teams: indiana_fever;toronto_tempo

Approved / candidate player assets included:
- Aliyah Boston | indiana_fever | uploaded file: assets/players/indiana_fever_aliyah_boston.png
- Bree Hall | indiana_fever | uploaded file: assets/players/indiana_fever_bree_hall.png
- Brittney Sykes | toronto_tempo | uploaded file: assets/players/toronto_tempo_brittney_sykes.png
- Isabelle Harrison | toronto_tempo | uploaded file: assets/players/toronto_tempo_isabelle_harrison.png

Non-negotiables:
- Use only uploaded assets.
- Do not fetch logos.
- Do not invent players.
- Do not invent stats.
- Do not use fake player photos.
- Do not use generic blank headshots.
- Do not create white dashboard cards.
- Do not render internal QA labels or workflow language.
- Keep all text readable and inside safe zones.
- Use the official HSD watermark only if included.
- Export each slide as a separate PNG.
- Send the finished images in a ZIP.

Use the premium HSD style:
dark cinematic sports-media background, bold condensed typography, strong team color accents, clean hierarchy, ESPN/BR energy, no clutter.

Caption to use after graphics are generated:
Tonight in the W. • Tonight in the W Two games, two different questions. Connecticut and Toronto is about who sets the tone early. LA and Seattle is about who can own the late window and make the fourth quarter loud. This is the kind of slate where the first run matters, but the last answer matters more. Which matchup are you watching first? 🏀 #HerSportsDaily #WomensSports #WNBA #Basketball #TorontoTempo

Manual prompt dossier:
# HSD Manual Prompt Dossier — Tonight in the W

Packet: `upload-pack_tonight-in-the-w`
Platform: `Manual Graphics Pack`
Family: `manual_graphics_pack`
Quality: `rich`

## Brand Style

Premium women’s sports editorial. Dark high-contrast background. Bold condensed sports typography. Cinematic lighting. Clean hierarchy. No white dashboard cards. No tiny text. Use the official HSD watermark exactly.

## Evidence

Tonight in the W | graphics_upload_pack | Tonight in the W | Tonight in the W | Games worth watching | Tonight in the W | Games worth watching | Tonight in the W | # Tonight in the W — Safe Graphics Prompt v2 Version: v3.2.17-mermaid-preview-player-lock-v2 Generated: 2026-06-16T04:22:19.891066+00:00 Create a premium Her Sports Daily 4-slide 1080x1350 carousel for Tonight in the W. CRITICAL PLAYER SAFETY LOCK: - Do not generate, invent, synthesize, redraw, or approximate any player/person image. - Use only exact uploaded player files that are explicitly named in this prompt. - Do not put a player in an opponent jersey or wrong team context. - Do not use a player image for any other player name. - If the player slide is not explicitly allowed below, do not use player images at all. Slate: - 

## Carousel Plan

### Slide 1: Cover
On-image copy: Tonight in the W
Visual direction: Premium HSD cover in the preferred dark cinematic style.

### Slide 2: Slate
On-image copy: Tonight in the W
Visual direction: Clean matchup board with official logos.

### Slide 3: What to watch
On-image copy: First run. Pace. Fourth-quarter answers.
Visual direction: Three editorial watch tiles.

### Slide 4: Keys
On-image copy: Pace. Glass. Shot quality. Late-clock answers.
Visual direction: Four bold key tiles.

### Slide 5: CTA
On-image copy: Which matchup are you watching first?
Visual direction: Poll-style HSD end slide.

## Caption

Tonight in the W.

• Tonight in the W

Two games, two different questions. Connecticut and Toronto is about who sets the tone early. LA and Seattle is about who can own the late window and make the fourth quarter loud.

This is the kind of slate where the first run matters, but the last answer matters more.

Which matchup are you watching first? 🏀

#HerSportsDaily #WomensSports #WNBA #Basketball #TorontoTempo

## Negative Prompt

No fake players. No invented stats. No unapproved headshots. No generic blank headshots. No cut-off names. No low-contrast text. No extra teams. No white panels.

