HSD GRAPHICS PRODUCTION.

Use the uploaded ZIP only.

Create premium Her Sports Daily graphics for:
Golden State Valkyries beat Los Angeles Sparks

Variant: logos_only
PLAYER MODE: LOGOS ONLY.
Do not use player photos even if present elsewhere. Build this as a logo-forward, scoreboard-forward, or text-forward HSD graphic.

Detected teams: golden_state_valkyries;los_angeles_sparks

Approved / candidate player assets included:
- None

Non-negotiables:
- Use only uploaded assets.
- Do not fetch logos.
- Do not invent players.
- Do not invent stats.
- Do not use fake player photos.
- Do not use generic blank headshots.
- Do not create white dashboard cards.
- Do not render internal QA labels or workflow language.
- Keep all text readable and inside safe zones.
- Use the official HSD watermark only if included.
- Export each slide as a separate PNG.
- Send the finished images in a ZIP.

Use the premium HSD style:
dark cinematic sports-media background, bold condensed typography, strong team color accents, clean hierarchy, ESPN/BR energy, no clutter.

Caption to use after graphics are generated:
Golden State Valkyries beat Los Angeles Sparks. Final: Golden State Valkyries 78, Los Angeles Sparks 58. Dallas did more than win. The Wings put a number on the board that makes the whole night feel different, and Vegas leaves with questions that will carry into the next one. That is the kind of result that deserves more than a score graphic. What did this one tell you? 🏀 #HerSportsDaily #WomensSports #WNBA #Basketball #GoldenStateValkyries #LASparks

Manual prompt dossier:
# HSD Manual Prompt Dossier — Golden State Valkyries beat Los Angeles Sparks

Packet: `ig-stories_ig-stories-rolling-1_golden-state-valkyries-beat-los-angeles-sparks`
Platform: `IG Stories`
Family: `wnba_result_recap`
Quality: `rich`

## Brand Style

Premium women’s sports editorial. Dark high-contrast background. Bold condensed sports typography. Cinematic lighting. Clean hierarchy. No white dashboard cards. No tiny text. Use the official HSD watermark exactly.

## Evidence

Golden State Valkyries beat Los Angeles Sparks | WNBA | result_or_recap | Golden State Valkyries beat Los Angeles Sparks: put this result on the board. | Golden State Valkyries beat Los Angeles Sparks The result matters because it changes the conversation, not just the scoreboard. | RESULT CHECK: Golden State Valkyries beat Los Angeles Sparks What changed after this one? | What was the swing moment? | v3.3.6-mermaid-assignment-handoff-v2.6 | ig_stories_rolling_1 | IG Stories | 10:30 AM | result_or_recap | Golden State Valkyries beat Los Angeles Sparks | WNBA | P1 | story_043920b88ad2a0 | ready_with_review | score_assets_needed | 78 | supporting_story_card_one | fresh | Golden State Valkyries beat Los Angeles Sparks: put this result on the board. | Golden State Valkyries beat Los Angeles Sp

## Carousel Plan

### Slide 1: Final
On-image copy: Golden State Valkyries beat Los Angeles Sparks
Visual direction: Final-score cover with official team logos.

### Slide 2: Score
On-image copy: Final: Golden State Valkyries 78, Los Angeles Sparks 58.
Visual direction: Large final score. Winner hierarchy clear.

### Slide 3: What changed
On-image copy: Dallas made the night louder.
Visual direction: Editorial takeaway slide.

### Slide 4: CTA
On-image copy: What did this one tell you?
Visual direction: Question-first HSD end slide.

## Caption

Golden State Valkyries beat Los Angeles Sparks.

Final: Golden State Valkyries 78, Los Angeles Sparks 58.

Dallas did more than win. The Wings put a number on the board that makes the whole night feel different, and Vegas leaves with questions that will carry into the next one.

That is the kind of result that deserves more than a score graphic.

What did this one tell you? 🏀

#HerSportsDaily #WomensSports #WNBA #Basketball #GoldenStateValkyries #LASparks

## Negative Prompt

No fake players. No invented stats. No unapproved headshots. No generic blank headshots. No cut-off names. No low-contrast text. No extra teams. No white panels.

