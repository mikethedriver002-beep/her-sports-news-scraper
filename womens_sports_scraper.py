"""
Her Sports Daily Women's Sports News Scraper v9
-----------------------------------------------
...
v9 fixes:
- Blocks obvious men's sports and unrelated NCAA stories before scoring.
- Uses word/phrase matching instead of loose substring matching.
- Treats soft viral items as filler, not must-post news.
- Stops "championship" by itself from making every story high priority.
- Requires explicit women's context for tennis, golf, baseball, and broad NCAA stories.
- Caps Must Post at 2 and the brief at 10.
"""
...
SPORT_RULES: List[Tuple[str, List[str]]] = [
    ...
    ("NCAA Women's Basketball", [
        ...
        "south carolina women", "usc women", "lsu women", "juju watkins",
    ]),
    ("NWSL / Women's Soccer", [
    ...
    ("Cricket", ["women's cricket", "women’s cricket", "women's t20", "women’s t20"]),
...
def womens_relevance_gate(text: str, source: str, sport: str) -> Tuple[bool, str]:
    ...
    if has_women_context or has_female_athlete or sport in {"WNBA", "NWSL / Women's Soccer", "PWHL / Women's Hockey"}:
...
def classify_story_type(text: str) -> str:
    ...
    if any_phrase(text, ["record", "historic", "first", "shatter", "shatters", "breaks record", "milestone"]):
        return "Record / Milestone"
    if any_phrase(text, ["revenue", "media rights", "sponsorship", "valuation", "investment", "ratings", "viewership", "attendance"]):
    ...
    if any_phrase(text, ["preview", "faces", "matchup", "schedule", "where to watch", "tonight", "tomorrow"]):
        return "Game Preview"
...
def freshness_component(article: Dict[str, str], story_type: str) -> Tuple[int, str]:
    ...
    fast_decay = {"Game Recap / Result", "Game Preview", "Breaking / Transaction", "Injury / Availability", "Rumor / Needs Verification"}
...
def story_value_component(text: str, story_type: str) -> Tuple[int, str]:
    if story_type == "Record / Milestone":
        return 32, "Record or milestone."
    if story_type == "Business / Growth":
        return 28, "Business/growth story."
    if story_type == "League Expansion":
        return 28, "Expansion story."
    if story_type == "Breaking / Transaction":
        return 24, "Confirmed breaking or transaction story."
    if story_type == "Game Recap / Result":
        if any_phrase(text, ["championship", "title", "final", "commissioner's cup"]):
            return 28, "Important result."
        return 20, "Game result."
    if story_type == "Game Preview":
        return 15, "Preview story."
...
def editorial_decision(article: Dict[str, str]) -> Tuple[str, str]:
    ...
    if score >= 88 and source_quality >= 8 and story_type in {"Record / Milestone", "Business / Growth", "League Expansion", "Breaking / Transaction", "Game Recap / Result"}:
...
def post_format(story_type: str, score: int) -> str:
    if story_type in {"Breaking / Transaction", "Game Recap / Result", "Record / Milestone"} and score >= 80:
