...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; HerSportsDailyResultsDesk/1.0; +https://github.com/)"
}
...
def get_scoreboard(source: Dict[str, str]) -> Dict[str, Any]:
    return request_json(scoreboard_url(source["sport"], source["league"]))
...
def build_game_row(source: Dict[str, str], event: Dict[str, Any], summary: Optional[Dict[str, Any]]) -> Tuple[GameRow, List[Dict[str, str]]]:
...
def main() -> None:
    ...
    for source in SOURCES:
        try:
            scoreboard = get_scoreboard(source)
        except Exception as exc:
            print(f"Failed scoreboard fetch for {source['label']}: {exc}")
            continue
        ...
        for event in scoreboard.get("events") or []:
            ...
            game_row, performers = build_game_row(source, event, summary)
    ...
    write_csv(
        ...
        seed_rows,
        [
            "league_label", "matchup", "game_state", "winner", "loser", "final_score", "headline",
            "top_performer_1", "top_performer_1_statline", "top_performer_2", "top_performer_2_statline",
