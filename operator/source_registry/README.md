...
Guided league proposal packs generate `source_registry_update_worksheet.csv/.md`, `source_registry_proposal_promotion_checklist.csv/.md`, `source_registry_proposal_draft.csv/.md`, `source_proposal_pack_readiness.csv/.md`, `source_proposal_packs.csv/.md`, plus focused files such as `wnba_source_proposal_pack.csv/.md`, `nwsl_source_proposal_pack.csv/.md`, `lpga_source_proposal_pack.csv/.md`, and `pwhl_source_proposal_pack.csv/.md` with specific free official league, team, tournament, and public cross-check candidates. Treat them as copy-ready review guides only; they do not enable sources, update the trusted registry, scrape private pages, call paid APIs, or publish. Use the checklist to decide which selected draft rows to verify/copy, hold, or discard, use the worksheet to compare review-only registry change plans with proposed disabled JSON and rollback notes, use the draft to review selected rows with duplicate/freshness warnings preserved, and use the readiness report to see which packs are ready for manual proposal review, need duplicate review, or need source freshness checks.
