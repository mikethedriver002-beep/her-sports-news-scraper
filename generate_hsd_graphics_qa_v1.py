...
def main():
    ...
    report=["# HSD Graphics QA Scorer v1.2.1 Report","",f"Generated: {now()}","",f"Bundles scored: {len(rows)}",""]
    if not rows:
    ...
    Path("graphics_qa_dashboard/index.html").write_text(f"<html><body><h1>Graphics QA v1.2.1</h1><p>Bundles scored: {len(rows)}</p></body></html>",encoding="utf-8")
