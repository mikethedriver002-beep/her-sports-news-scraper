...
def main():
    ...
    for b in bundles:
        ...
        if b.get("post_slug") == "main-wnba-result":
            for language_file in ["graphics_copy_style_guide.md", "graphics_display_copy.csv", "graphics_banned_language.csv", "graphics_asset_usage_map.csv"]:
                if not Path(language_file).exists():
                    issues.append({"code":"LANGUAGE_PACK_MISSING","severity":"major","message":language_file}); score-=8
    ...
    report=["# HSD Graphics QA Scorer v1.6 Report","",f"Generated: {now()}","",f"Bundles scored: {len(rows)}",""]
    if not rows:
    ...
    Path("graphics_qa_dashboard/index.html").write_text(f"<html><body><h1>Graphics QA v1.6</h1><p>Bundles scored: {len(rows)}</p></body></html>",encoding="utf-8")
