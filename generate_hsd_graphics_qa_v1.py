...
VERSION="hsd-graphics-qa-scorer-v1.1"
...
def main():
    ...
    for b in bundles:
        ...
        rows.append({"qa_run_id":run,"bundle_id":b.get("bundle_id"),"post_slug":b.get("post_slug"),"template_name":b.get("template_name"),"render_path":b.get("render_path"),"score_total":max(0,score),"critical_fail":"Yes" if decision=="fail" else "No","decision":decision,"issues_json":json.dumps(issues),"remediation_suggestions":"Generate/export graphic and rerun QA. Keep exact assets only.","checked_utc":now()})
    write_csv("graphics_qa_results.csv",rows,FIELDS)
    report=["# HSD Graphics QA Scorer v1.1 Report","",f"Generated: {now()}","",f"Bundles scored: {len(rows)}",""]
    if not rows:
        report += ["No bundles found in render manifest. Run Visual Upgrade v2.1 first."]
    for r in rows:
    ...
    Path("graphics_qa_dashboard/index.html").write_text(f"<html><body><h1>Graphics QA v1.1</h1><p>Bundles scored: {len(rows)}</p></body></html>",encoding="utf-8")
    print("Created HSD Graphics QA v1.1 outputs")
