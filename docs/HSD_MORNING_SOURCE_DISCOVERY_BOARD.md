...
- Guided league proposal packs produce `source_registry_update_worksheet.csv/.md`, `source_registry_proposal_promotion_checklist.csv/.md`, `source_registry_proposal_draft.csv/.md`, `source_proposal_pack_readiness.csv/.md`, `source_proposal_packs.csv/.md`, plus focused pack files such as `wnba_source_proposal_pack.csv/.md`, `nwsl_source_proposal_pack.csv/.md`, `lpga_source_proposal_pack.csv/.md`, and `pwhl_source_proposal_pack.csv/.md`. The checklist tells the operator which selected draft rows to verify/copy, hold, or discard before any trusted registry edit. The worksheet turns verify/copy rows into review-only registry change plans with proposed disabled JSON, before/after notes, and rollback notes. The draft file stages selected pack rows with duplicate/freshness warnings preserved; the readiness report tells the operator which packs are ready for manual registry proposal review, need duplicate review, or need source freshness checks. These are curated free public official, team, tournament, and cross-check candidates for manual review only. The worksheet, checklist, draft, and packs never import rows, enable sources, scrape private pages, use paid APIs, or publish.
