...
- Guided league proposal packs produce `source_proposal_pack_readiness.csv/.md`, `source_proposal_packs.csv/.md`, plus focused pack files such as `wnba_source_proposal_pack.csv/.md`, `nwsl_source_proposal_pack.csv/.md`, `lpga_source_proposal_pack.csv/.md`, and `pwhl_source_proposal_pack.csv/.md`. The readiness report tells the operator which packs are ready for manual registry proposal review, need duplicate review, or need source freshness checks. These are curated free public official, team, tournament, and cross-check candidates for manual review only. The packs never import rows, enable sources, scrape private pages, use paid APIs, or publish.
