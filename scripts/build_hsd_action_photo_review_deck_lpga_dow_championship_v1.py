...
VERSION = "hsd-action-photo-review-deck-lpga-dow-championship-v1-review-only"
DEFAULT_LPGA_DOW_CHAMPIONSHIP_CSV = Path(
    "outputs/local/latest/files/action_photo_lpga_dow_championship_source_expansion_v1/action_photo_candidate_intake.csv"
)
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/action_photo_review_deck_lpga_dow_championship_v1")
DEFAULT_LIMIT = 24


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local review-only action-photo deck from LPGA Dow Championship source candidates."
    )
    parser.add_argument("--board-csv", default=DEFAULT_LPGA_DOW_CHAMPIONSHIP_CSV.as_posix())
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest["source_packet"] = "action_photo_lpga_dow_championship_source_expansion_v1"
