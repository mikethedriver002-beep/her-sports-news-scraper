...
VERSION = "hsd-wnba-fever-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_fever_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_fever_source_scout_v1"
DEFAULT_DECK_OUTPUT_DIR = DEFAULT_OUTPUT_DIR / "review_deck"
DEFAULT_USER_AGENT = "HSD WNBA Fever Source Scout/1.0 (review-only metadata scout)"
...
def source_quality_score(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    ...
    if candidate_url:
        score += 10
    if "kelsey mitchell" in lower_title:
        score += 10
    else:
        score -= 2
    if "kelsey mitchell" in lower_description:
        score += 8
    if any(term in lower_title for term in ("game recap", "power", "win", "defeat", "dominate", "thriller")):
        score += 4
    if any(token in lower_url for token in ("gettyimages", "dsc_", "pse_", "img_")):
        score += 6
    if "fever.wnba.com" in lower_url:
        score += 3
    if paywall_hit == "true":
        score -= 12
        flags.append("paywall_marker_detected")
    if "kelsey mitchell" not in lower_title and "kelsey mitchell" not in lower_description:
        flags.append("subject_context_not_explicit")
        score -= 6
    if not candidate_url:
        flags.append("missing_candidate_image_url")
        score -= 25
    return max(0, min(100, score)), ("A_primary_source_lead" if score >= 90 else "B_strong_source_lead" if score >= 80 else "C_secondary_source_lead"), flags
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"WFFS{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Fever recap page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": "https://www.wnba.com/player/1628909/kelsey-mitchell",
                ...
                "manual_next_action": "Open the official recap, confirm the OG image is a clean action-frame lead, and carry forward only if it still reads as a strong WNBA source-quality candidate.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_fever_official_galleries_and_recaps",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": clean(seed.get("entity_id")),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    strongest = rows[:3]
    lines = [
        "# WNBA Fever Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Indiana Fever recap/galleries lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    scout_seed_path = output_dir / "wnba_fever_source_scout_seed.csv"
    intake_path = output_dir / "wnba_fever_source_scout_intake.csv"
    board_path = output_dir / "wnba_fever_source_scout_board.csv"
    report_path = output_dir / "wnba_fever_source_scout_report.md"
    ...
    manifest = {
        ...
        "status": "wnba_fever_source_scout_ready" if intake_rows else "wnba_fever_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the WNBA Fever official galleries and recaps source scout packet.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest = build_packet(seed_csv=resolve_path(args.seed_csv), output_dir=output_dir)
    print(
        json.dumps(
            {
