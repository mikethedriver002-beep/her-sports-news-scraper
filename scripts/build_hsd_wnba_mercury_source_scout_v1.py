...
VERSION = "hsd-wnba-mercury-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_mercury_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v2"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v2"
DEFAULT_USER_AGENT = (
...
TINY_THUMBNAIL_RE = re.compile(r"-(?:185x148|260x190|320x180|640x360|1024x576|1280x720)(?=\.)", re.I)
...
def is_useful_mercury_image_url(url: str) -> bool:
    parsed = urlparse(clean(url))
    ...
    if is_tiny_thumbnail_url(parsed.geturl()):
        return False
    if any(token in lowered for token in ("favicon", "apple-touch-icon", "cropped-mercuryprimary", "/static/next/")):
        return False
    return any(token in lowered for token in ("gettyimages-", "phxvssparks", "-scaled"))
...
def extract_image_urls(page_url: str, text: str, parser: PageParser) -> list[str]:
    ...
    for raw in candidates:
        ...
        if candidate in seen:
            continue
        seen.add(candidate)
        if is_useful_mercury_image_url(candidate):
            useful.append(candidate)
    no_query_preferred = [url for url in useful if "?" not in url]
    return no_query_preferred or useful
...
def score_row(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    ...
    if "game gallery" in combined or "gallery" in combined:
        score += 14
    if "mercury" in combined or "mercury" in url:
        score += 8
    ...
    flags.append("matchup_level_identity_only")
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"WMGS{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Mercury game-gallery page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://mercury.wnba.com/roster"),
                ...
                "manual_next_action": "Open the Mercury game gallery, confirm the selected high-resolution image is real action-photo evidence, and carry forward only if the crop works for review-only 4:5 intake.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_mercury_official_game_galleries",
                "candidate_queue_id": candidate_id,
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# WNBA Mercury Official Game Gallery Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Phoenix Mercury game-gallery lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    seed_path = output_dir / "wnba_mercury_source_scout_seed.csv"
    intake_path = output_dir / "wnba_mercury_source_scout_intake.csv"
    board_path = output_dir / "wnba_mercury_source_scout_board.csv"
    report_path = output_dir / "wnba_mercury_source_scout_report.md"
    ...
    manifest: dict[str, Any] = {
        ...
        "status": "wnba_mercury_source_scout_ready" if intake_rows else "wnba_mercury_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Phoenix Mercury official game-gallery source scout packet.")
