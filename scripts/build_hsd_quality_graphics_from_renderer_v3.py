...
VERSION = "v1.0-quality-from-template-renderer-v3"
...
FIELDS = ["event_id","platform","row_kind","headline","variant","player_mode","player_names","player_assets_used","renderer_source","output_path","width","height","used_home_logo","used_away_logo","status","notes"]
...
def loaded_teams() -> set[str]:
    try:
        ...
        return set()
    return {norm(row.get("team")) for row in payload.get("rows", []) if row.get("status") == "loaded"}


def main() -> None:
    contract = {clean(row.get("event_id")): row for row in read_csv(CONTRACT) if clean(row.get("event_id"))}
    loaded = loaded_teams()
    if OUT.exists():
        shutil.rmtree(OUT)
    ...
    for row in read_csv(V3_MANIFEST):
        ...
        home = clean(src.get("home_team_name"))
        away = clean(src.get("away_team_name"))
        promoted.append({
            "event_id": event_id,
            "platform": platform,
            "row_kind": clean(src.get("row_kind")),
            ...
            "used_home_logo": "yes" if norm(home) in loaded else "no",
            "used_away_logo": "yes" if norm(away) in loaded else "no",
            "status": "rendered",
            "notes": "Cinematic renderer v3 output; Phase 5 visual approval required",
        })
    ...
    REPORT.write_text("\n".join([
        "# HSD Quality Graphics — Renderer v3",
        "",
    ...
    print(json.dumps({"version": VERSION, "promoted": len(promoted), "logos_only": logos_only, "with_players": with_players}, indent=2))
