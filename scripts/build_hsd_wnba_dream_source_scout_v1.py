...
VERSION = "hsd-wnba-dream-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_dream_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v4"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v4"
DEFAULT_USER_AGENT = (
...
def is_useful_dream_image_url(url: str) -> bool:
    parsed = urlparse(clean(url))
    lowered = parsed.geturl().lower()
    if parsed.netloc != "cdn.wnba.com":
        return False
    if "/sites/1611661330/" not in parsed.path:
        return False
    ...
    if any(token in lowered for token in ("favicon", "apple-touch-icon", "/static/next/", "/headshots/")):
        return False
    return any(token in lowered for token in ("story", "photo", "rhyne", "jordin", "untitled-22"))
...
def extract_image_urls(page_url: str, text: str, parser: PageParser) -> list[str]:
    ...
    for raw in candidates:
        ...
        if candidate in seen:
            continue
        seen.add(candidate)
        if is_useful_dream_image_url(candidate):
...
def score_row(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    ...
    if "recap" in combined or "win" in combined or "defense" in combined or "dream" in combined:
        score += 10
    if "dream" in combined or "atlanta" in combined or "dream" in url:
        score += 8
    if any(token in url for token in ("story", "photo", "rhyne", "jordin")):
        score += 8
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"DRM{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Dream game-recap page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://dream.wnba.com/roster"),
                ...
                "manual_next_action": "Open the Dream recap, confirm the selected high-resolution feature image is real game-action evidence, and carry forward only if the 4:5 crop works for review-only intake.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_dream_official_game_recaps",
                "candidate_queue_id": candidate_id,
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# WNBA Dream Official Game Recap Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Atlanta Dream game-recap feature-image lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    seed_path = output_dir / "wnba_dream_source_scout_seed.csv"
    intake_path = output_dir / "wnba_dream_source_scout_intake.csv"
    board_path = output_dir / "wnba_dream_source_scout_board.csv"
    report_path = output_dir / "wnba_dream_source_scout_report.md"
    ...
    manifest: dict[str, Any] = {
        ...
        "status": "wnba_dream_source_scout_ready" if intake_rows else "wnba_dream_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Atlanta Dream official recap source scout packet.")
