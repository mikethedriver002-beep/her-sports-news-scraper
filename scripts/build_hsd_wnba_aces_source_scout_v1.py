...
VERSION = "hsd-wnba-aces-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_aces_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_team_source_scout_v1"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_team_source_scout_v1"
DEFAULT_DECK_OUTPUT_DIR = DEFAULT_OUTPUT_DIR / "review_deck"
DEFAULT_USER_AGENT = "HSD WNBA Aces Source Scout/1.0 (review-only metadata scout)"
...
def source_quality_score(
    ...
    score = 66
    flags: list[str] = []
    lower_title = searchable(title)
    lower_description = searchable(description)
    lower_url = searchable(candidate_url)
    lower_hint = searchable(subject_hint)
    if candidate_url:
        score += 10
    if "aces" in lower_title or "aces" in lower_description or "aces" in lower_url:
        score += 6
    if any(term in lower_title for term in ("game recap", "recap", "playoffs", "gallery")):
        score += 4
    if lower_hint and (lower_hint in lower_title or lower_hint in lower_description):
        ...
        score -= 4
    if any(token in lower_url for token in ("gettyimages", "dsc_", "pse_", "img_")):
        score += 6
    if paywall_hit == "true":
        score -= 12
    ...
    if not candidate_url:
        ...
        score -= 25
    return max(0, min(100, score)), ("A_primary_source_lead" if score >= 88 else "B_strong_source_lead" if score >= 78 else "C_secondary_source_lead"), flags
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        confidence = clean(seed.get("identity_confidence") or "strong_context")
        candidate_id = f"WAFS{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Aces recap page.")
        notes = (
        ...
        identity_anchor_url = clean(seed.get("identity_anchor_url") or "https://aces.wnba.com/roster")
        intake_rows.append(
            {
                ...
                "manual_next_action": "Open the Aces recap, confirm the OG image still supports a clean solo or near-solo action lead, and carry forward only if the crop still reads as a usable 4:5 review candidate.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_aces_official_recaps",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": clean(seed.get("entity_id")),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    strongest = rows[:3]
    lines = [
        "# WNBA Aces Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Las Vegas Aces recap lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    scout_seed_path = output_dir / "wnba_aces_source_scout_seed.csv"
    intake_path = output_dir / "wnba_aces_source_scout_intake.csv"
    board_path = output_dir / "wnba_aces_source_scout_board.csv"
    report_path = output_dir / "wnba_aces_source_scout_report.md"
    ...
    manifest = {
        ...
        "status": "wnba_aces_source_scout_ready" if intake_rows else "wnba_aces_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Las Vegas Aces official recaps source scout packet.")
