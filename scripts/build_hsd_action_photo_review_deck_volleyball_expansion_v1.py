...
VERSION = "hsd-action-photo-review-deck-volleyball-expansion-v1-review-only"
DEFAULT_VOLLEYBALL_EXPANSION_CSV = Path(
    "outputs/local/latest/files/action_photo_volleyball_source_expansion_v1/action_photo_candidate_intake.csv"
)
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/action_photo_review_deck_volleyball_expansion_v1")
DEFAULT_LIMIT = 36


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local review-only action-photo deck from the volleyball source expansion candidates."
    )
    parser.add_argument("--board-csv", default=DEFAULT_VOLLEYBALL_EXPANSION_CSV.as_posix())
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest["source_packet"] = "action_photo_volleyball_source_expansion_v1"
