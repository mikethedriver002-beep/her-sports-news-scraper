...
VERSION = "hsd-wnba-sparks-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_sparks_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v9"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v9"
DEFAULT_USER_AGENT = (
...
def is_useful_sparks_image_url(url: str) -> bool:
    parsed = urlparse(clean(url))
    lowered = parsed.geturl().lower()
    if parsed.netloc != "cdn.wnba.com":
        return False
    if "/sites/1611661320/" not in parsed.path:
        return False
...
def extract_image_urls(page_url: str, text: str, parser: PageParser) -> list[str]:
    ...
    for raw in candidates:
        ...
        if candidate in seen:
            continue
        seen.add(candidate)
        if is_useful_sparks_image_url(candidate):
...
def source_quality_score(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    ...
    if any(term in lower_title for term in ("game recap", "recap", "preview", "home opener")):
        score += 10
    if "sparks" in lower_title or "sparks" in lower_description or "sparks" in lower_url:
        score += 6
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"LSP{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Sparks recap page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://sparks.wnba.com/roster"),
                ...
                "manual_next_action": "Open the official recap, confirm the OG image is a real high-res game-action frame, and carry forward only if it still reads as a strong review-only candidate.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_sparks_official_game_recaps_highres",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": clean(seed.get("entity_id")),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# WNBA Sparks Official Game Recap Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Los Angeles Sparks game-recap lane.",
        "",
        "## Summary",
        "",
    ...
    if strongest:
        ...
        for row in strongest:
            lines.append(
                f"| {row['board_rank']} | {row['candidate_queue_id']} | {row['score']} | {row['candidate_quality_tier']} | {row['source_url']} | Open the recap and confirm the OG image still reads like a true action lead. |"
            )
    else:
...
def build_packet(
    *,
    ...
    seed_path = output_dir / "wnba_sparks_source_scout_seed.csv"
    intake_path = output_dir / "wnba_sparks_source_scout_intake.csv"
    board_path = output_dir / "wnba_sparks_source_scout_board.csv"
    report_path = output_dir / "wnba_sparks_source_scout_report.md"
    ...
    source_family_usefulness_verdict = (
        "useful_high_res_official_recap_family"
        if board_rows and thumbnail_suffix_count == 0
        else "mixed_or_noisy_official_family"
    )
    strengths = [
        "Official Sparks recap pages are public and reachable without login.",
        "High-res CDN image URLs are present and live, not thumbnail-only placeholders.",
        "The lane is matchup-scoped and stays review-only without overclaiming player identity.",
    ]
    weaknesses = [
        "Some recap pages still lean on matchup-level identity rather than named-player proof.",
        "This family is not a guaranteed action-frame win on every recap, so manual review still matters.",
    ]
    manifest = {
        "version": VERSION,
        "generated_by": GENERATED_BY,
        "generated_at_utc": now_iso(),
        "status": "wnba_sparks_source_scout_ready" if intake_rows else "wnba_sparks_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Los Angeles Sparks official game-recap source scout packet.")
