...
VERSION = "hsd-wnba-storm-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_storm_source_scout_v1.py"
...
DEFAULT_USER_AGENT = "HSD WNBA Storm Source Scout/1.0 (review-only metadata scout)"
...
def subject_phrase(entity_id: str) -> str:
    parts = [part for part in clean(entity_id).split("_") if part]
    if len(parts) >= 4:
        return " ".join(parts[3:])
    return " ".join(parts)
...
def source_quality_score(
    ...
    if candidate_url:
        score += 10
    if "storm" in lower_title or "storm" in lower_description or "storm" in lower_url:
        score += 6
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"WSFS{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Storm recap page.")
        notes = (
        ...
        identity_anchor_url = clean(seed.get("identity_anchor_url") or "https://storm.wnba.com/roster")
        intake_rows.append(
            {
                ...
                "manual_next_action": "Open the Storm recap, confirm the OG image still supports a clean solo or near-solo action lead, and carry forward only if the crop still reads as a usable 4:5 review candidate.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_storm_official_recaps",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": clean(seed.get("entity_id")),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    strongest = rows[:3]
    lines = [
        "# WNBA Storm Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Seattle Storm recap lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    scout_seed_path = output_dir / "wnba_storm_source_scout_seed.csv"
    intake_path = output_dir / "wnba_storm_source_scout_intake.csv"
    board_path = output_dir / "wnba_storm_source_scout_board.csv"
    report_path = output_dir / "wnba_storm_source_scout_report.md"
    ...
    manifest = {
        ...
        "status": "wnba_storm_source_scout_ready" if intake_rows else "wnba_storm_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Seattle Storm official recaps source scout packet.")
