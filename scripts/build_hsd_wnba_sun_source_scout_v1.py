...
VERSION = "hsd-wnba-sun-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_sun_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v6"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v6"
DEFAULT_USER_AGENT = (
...
def is_useful_sun_image_url(url: str) -> bool:
    parsed = urlparse(clean(url))
    lowered = parsed.geturl().lower()
    if parsed.netloc != "cdn.wnba.com":
        return False
    if "/sites/1611661323/" not in parsed.path:
        return False
...
def extract_image_urls(page_url: str, text: str, parser: PageParser) -> list[str]:
    ...
    for raw in candidates:
        ...
        if candidate in seen:
            continue
        seen.add(candidate)
        if is_useful_sun_image_url(candidate):
...
def score_row(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    ...
    if "recap" in combined or "win" in combined or "defeat" in combined or "sun" in combined:
        score += 10
    if "sun" in combined or "connecticut" in combined or "sun" in url:
        score += 8
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"SUN{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Sun game-recap page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://sun.wnba.com/roster"),
                ...
                "manual_next_action": "Open the Sun recap, confirm the selected high-resolution feature image is real game-action evidence, and carry forward only if the 4:5 crop works for review-only intake.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_sun_official_game_recaps",
                "candidate_queue_id": candidate_id,
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# WNBA Sun Official Game Recap Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Connecticut Sun game-recap high-resolution image lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    seed_path = output_dir / "wnba_sun_source_scout_seed.csv"
    intake_path = output_dir / "wnba_sun_source_scout_intake.csv"
    board_path = output_dir / "wnba_sun_source_scout_board.csv"
    report_path = output_dir / "wnba_sun_source_scout_report.md"
    ...
    manifest: dict[str, Any] = {
        ...
        "status": "wnba_sun_source_scout_ready" if intake_rows else "wnba_sun_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Connecticut Sun official recap source scout packet.")
