...
VERSION = "hsd-wnba-sky-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_sky_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v8"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v8"
...
def normalize_wnba_cdn_image_url(image_url: str) -> tuple[str, bool]:
    ...
    high_res_path = stem
    normalized = False
    for suffix in ("-185x148", "-260x190", "-320x180", "-640x360"):
...
def subject_phrase(entity_id: str) -> str:
    ...
    for prefix in ("wnba_minnesota_lynx_", "wnba_chicago_sky_"):
...
def source_quality_score(
    ...
    if candidate_url:
        score += 10
    if any(term in lower_title for term in ("photo gallery", "gallery", "game day highlights", "highlights", "photos", "in photos")):
        score += 14
    if any(term in lower_title or term in lower_description or term in lower_url for term in ("lynx", "chicago sky", "skytown")):
        score += 6
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"WSKY{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Chicago Sky gallery page.")
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://sky.wnba.com/roster"),
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_chicago_sky_official_game_day_highlights",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": clean(seed.get("entity_id")),
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    strongest = rows[:3]
    thumbnail_rows = [row for row in rows if "thumbnail_url_promoted_to_high_res" in row.get("candidate_risk_flags", "")]
    high_res_rows = [row for row in rows if row.get("candidate_image_url", "").startswith("https://cdn.wnba.com/")]
    lines = [
        "# WNBA Chicago Sky High-Resolution Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Chicago Sky game-day highlights lane.",
        "",
        "## Summary",
        "",
        f"- Seed rows: `{manifest['seed_row_count']}`",
        f"- Candidate rows: `{manifest['candidate_row_count']}`",
        f"- High-res image rows: `{len(high_res_rows)}`",
        f"- Thumbnail suffix hits: `{len(thumbnail_rows)}`",
    ...
    lines.extend(
        [
            ...
            "## Strengths",
            "",
            "- Public official Sky pages are reachable without auth bypass in this lane.",
            "- The image URLs resolve to CDN-hosted high-resolution files rather than tiny thumbnails.",
            "- Matchup and gallery context is clear enough for manual review without overclaiming named-player identity.",
            "",
            "## Weaknesses",
            "",
            "- Several pages are title-heavy highlight pages, so the metadata is strong for source scouting but still needs human visual confirmation.",
            "- Identity confidence stays source-scoped instead of player-specific, which is honest but less decisive than a named-player lane.",
            "",
            "## Guardrails",
            "",
            "- review_only=true",
...
def build_packet(
    *,
    ...
    scout_seed_path = output_dir / "wnba_sky_source_scout_seed.csv"
    intake_path = output_dir / "wnba_sky_source_scout_intake.csv"
    board_path = output_dir / "wnba_sky_source_scout_board.csv"
    report_path = output_dir / "wnba_sky_source_scout_report.md"
    ...
    manifest = {
        ...
        "status": "wnba_sky_source_scout_ready" if intake_rows else "wnba_sky_source_scout_empty",
        "review_only": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Chicago Sky official high-resolution source scout packet.")
