...
VERSION = "hsd-wnba-drm002-photoshop-finish-v2-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_drm002_photoshop_finish_v2.py"
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/wnba_drm002_photoshop_finish_v2")
DEFAULT_LATEST_OUTPUT_DIR = Path("outputs/local/latest/files/wnba_drm002_photoshop_finish_v2")
...
BUNDLE_NAME = "wnba_drm002_photoshop_finish_v2_bundle.zip"


VARIANT_SPECS: list[dict[str, Any]] = [
    {
        "variant_id": "drm002_raw_friction",
        "variant_name": "DRM002 Raw Friction",
        "output_base": "variant_01_drm002_raw_friction",
        "scale_height": 1600,
        "crop_center": 0.73,
        "crop_top": 118,
        "kicker": "CASE STUDY",
        ...
        "footer_tracking": 125,
        "decision": "keep",
        "note": "strongest swing; tightest friction crop and the most premium type block",
    },
    {
        "variant_id": "drm002_unyielding_motion",
        "variant_name": "DRM002 Unyielding Motion",
        "output_base": "variant_02_drm002_unyielding_motion",
        "scale_height": 1560,
        "crop_center": 0.77,
        "crop_top": 86,
        "kicker": "SOURCE AUDIT",
        ...
        "footer_tracking": 125,
        "decision": "keep",
        "note": "cleaner than raw friction but still forceful; best backup if the lead feels too loud",
    },
    {
        "variant_id": "drm002_breaks_frame",
        "variant_name": "DRM002 Breaks Frame",
        "output_base": "variant_03_drm002_breaks_frame",
        "scale_height": 1640,
        "crop_center": 0.80,
        "crop_top": 126,
        ...
        "footer_tracking": 120,
        "decision": "kill",
        "note": "interesting crop but the referee intrusion and right-edge pressure make it the weakest finish",
    },
]


def load_v1_module():
    script_path = Path(__file__).resolve().with_name("build_hsd_wnba_drm002_photoshop_finish_v1.py")
    spec = importlib.util.spec_from_file_location("build_hsd_wnba_drm002_photoshop_finish_v1", script_path)
...
function finishOne(job) {{
  ...
  addTextLayer(src, "kicker", job.kicker, 72, job.kickerTop, job.kickerSize, job.kickerTracking, job.kickerSize + 2, false);
  addTextLayer(
    src,
    "headline",
  ...
  addTextLayer(src, "footer", job.footer, 72, 1292, job.footerSize, job.footerTracking, job.footerSize + 2, false);
...
def build_contact_sheet(output_dir: Path, rows: list[dict[str, Any]]) -> Path:
    ...
    draw.text((34, 26), "WNBA DRM002 PHOTOSHOP FINISH V2", fill=(245, 246, 248), font=title_font)
    draw.text((34, 68), "Aggressive editorial pass from the local DRM002 quarantine asset. Review-only. No downloads, approvals, or publishing.", fill=(184, 192, 205), font=small_font)
...
def build_layer_map(output_dir: Path, rows: list[dict[str, Any]], source_path: Path) -> Path:
    lines = [
        "# DRM002 Photoshop layer map v2",
        "",
        f"- source: {source_path.as_posix()}",
        f"- layered working reference: {(output_dir / 'working' / 'drm002_photoshop_finish_v2_master.psd').as_posix()}",
        "",
        "## Variants",
    ]
    for row in rows:
...
def build_report(manifest: dict[str, Any]) -> str:
    ...
    return f"""# WNBA DRM002 Photoshop Finish Pass V2

Status: `{manifest['status']}`
Version: `{manifest['version']}`

This is a review-only Photoshop-first editorial v2 pass from the local DRM002 quarantine asset.

## Blunt Read

- Best premium route: `{manifest['best_variant_id']}`
- Keep: `drm002_raw_friction`, `drm002_unyielding_motion`
- Kill: `drm002_breaks_frame`
- Verdict: SALVAGE THE LEAD, DROP THE WEAK THIRD
- Why: the new path finally breaks the polite sports-graphic symmetry, but only the first two comps feel expensive enough to carry forward.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a review-only Photoshop finish v2 for DRM002.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    jsx_path = working_dir / "photoshop_finish_v2.jsx"
    ...
    if completed.returncode != 0 or not result_path.exists():
        raise RuntimeError(f"Photoshop finish v2 failed: stdout={completed.stdout.strip()} stderr={completed.stderr.strip()}")
    ...
    manifest: dict[str, Any] = {
        ...
        "status": "wnba_drm002_photoshop_finish_v2_ready",
        ...
        "best_variant_id": "drm002_raw_friction",
        "variant_rows": rows,
