...
GENERATED_BY = "scripts/build_hsd_wnba_drm002_photoshop_finish_v4.py"
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/wnba_drm002_photoshop_finish_v4")
DEFAULT_LATEST_OUTPUT_DIR = Path("outputs/local/latest/files/wnba_drm002_photoshop_finish_v4")
...
BUNDLE_NAME = "wnba_drm002_photoshop_finish_v4_bundle.zip"
...
VARIANT_SPECS: list[dict[str, Any]] = [
    {
        ...
        "output_base": "variant_01_drm002_merge_candidate",
        "scale_height": 1600,
        "crop_center": 0.736,
        "crop_top": 118,
        ...
        "footer_left": 76,
        "decision": "keep",
        "note": "best merge candidate; strongest balance of tension, spacing, and expensive restraint",
    },
    {
        "variant_id": "drm002_merge_candidate_soft",
        "variant_name": "DRM002 Merge Candidate Soft",
        "output_base": "variant_02_drm002_merge_candidate_soft",
        "scale_height": 1600,
        "crop_center": 0.74,
        "crop_top": 118,
        ...
        "footer_left": 76,
        "decision": "proof_only",
        "note": "proof-only alternate; elegant and useful for comparison, but not part of the default merge route",
    },
    {
        "variant_id": "drm002_merge_candidate_dense",
        "variant_name": "DRM002 Merge Candidate Dense",
        "output_base": "variant_03_drm002_merge_candidate_dense",
        "scale_height": 1600,
        "crop_center": 0.734,
        "crop_top": 118,
        "kicker": "LUXURY PROOF",
        ...
        "footer_left": 76,
        "decision": "kill",
        "note": "still good, but the denser type color loses the elegance we wanted from the final route",
    },
]
...
function addBottomShadow(doc, opacityPercent) {{
  var layer = doc.artLayers.add();
  layer.name = "bottom_shadow";
  doc.activeLayer = layer;
  doc.selection.select([
      [new UnitValue(0, "px"), new UnitValue(930, "px")],
      [new UnitValue(1080, "px"), new UnitValue(930, "px")],
...
function finishOne(job) {{
  ...
  addBottomShadow(src, 15);
  addTextLayer(src, "kicker", job.kicker, 78, job.kickerTop, job.kickerSize, job.kickerTracking, job.kickerSize + 2, false);
  addTextLayer(
    src,
    "headline",
  ...
  addTextLayer(src, "footer", job.footer, job.footerLeft, 1291, job.footerSize, job.footerTracking, job.footerSize + 2, false);
...
def build_layer_map(output_dir: Path, rows: list[dict[str, Any]], source_path: Path) -> Path:
    lines = [
        "# DRM002 Photoshop layer map v4",
        "",
        f"- source: {source_path.as_posix()}",
        f"- layered working reference: {(output_dir / 'working' / 'drm002_photoshop_finish_v4_master.psd').as_posix()}",
        "",
        "## Lead-only polish variants",
    ]
    for row in rows:
...
This is a review-only Photoshop-first final luxury polish pass from the local DRM002 quarantine asset.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a review-only Photoshop finish v4 for DRM002.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    jsx_path = working_dir / "photoshop_finish_v4.jsx"
    ...
    if completed.returncode != 0 or not result_path.exists():
        raise RuntimeError(f"Photoshop finish v4 failed: stdout={completed.stdout.strip()} stderr={completed.stderr.strip()}")
