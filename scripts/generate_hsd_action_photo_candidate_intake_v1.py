...
OUT_MANUAL_SOURCE_HUNT_BOARD_CSV = output_path(ROOT / "review_only_action_photo_manual_source_hunt_board_v1.csv")
OUT_MANUAL_SOURCE_HUNT_BOARD_MD = output_path(ROOT / "review_only_action_photo_manual_source_hunt_board_v1.md")
OUT_MANUAL_SOURCE_HUNT_BOARD_JSON = output_path(ROOT / "review_only_action_photo_manual_source_hunt_board_v1.json")
...
OUT_RESEARCH_RETURN_PASTE_WORKSHEET_CSV = output_path(ROOT / "review_only_action_photo_research_return_paste_worksheet_v1.csv")
OUT_RESEARCH_RETURN_PASTE_WORKSHEET_MD = output_path(ROOT / "review_only_action_photo_research_return_paste_worksheet_v1.md")
OUT_RESEARCH_RETURN_PASTE_WORKSHEET_JSON = output_path(ROOT / "review_only_action_photo_research_return_paste_worksheet_v1.json")
...
OUT_QUALITY_FIT_OPERATOR_CUE_CSV = output_path(ROOT / "review_only_action_photo_quality_fit_operator_cue_v1.csv")
OUT_QUALITY_FIT_OPERATOR_CUE_MD = output_path(ROOT / "review_only_action_photo_quality_fit_operator_cue_v1.md")
OUT_QUALITY_FIT_OPERATOR_CUE_JSON = output_path(ROOT / "review_only_action_photo_quality_fit_operator_cue_v1.json")
...
ACTION_PHOTO_MANUAL_SOURCE_HUNT_BOARD_FIELDS = [
    "source_hunt_id",
    "candidate_queue_id",
    "sport",
    "league_entity",
    ...
    "rights_review_category",
    "primary_manual_search_query",
    "secondary_manual_search_query",
    "candidate_page_url_guidance",
    "official_page_or_identity_anchor_needed",
    "candidate_photo_page_needed",
    "evidence_url_needed",
    "rights_class_hint",
    "paste_target",
    "paste_fields_to_fill",
    "blocked_or_deprioritized_sources",
    "red_flags_or_hold_reasons",
    "manual_next_action",
    "download_approved",
    "source_url",
    "entity_id",
...
ACTION_PHOTO_RESEARCH_RETURN_PASTE_WORKSHEET_FIELDS = [
    "paste_worksheet_id",
    "candidate_queue_id",
    "sport",
    "league_entity",
    ...
    "manual_reviewer",
    "manual_review_status",
    "missing_fields_before_candidate_ready",
    "candidate_ready_for_later_human_download_decision_review",
    "placeholder_example_cue",
    "manual_next_action",
    "download_approved",
    "review_only",
    "publish_ready",
...
ACTION_PHOTO_RESEARCH_RETURN_SUMMARY_BOARD_FIELDS = [
    "summary_id",
    "candidate_queue_id",
    "return_status",
    "operator_priority",
...
ACTION_PHOTO_QUALITY_FIT_OPERATOR_CUE_FIELDS = [
    "operator_cue_id",
    "quality_fit_id",
    "candidate_queue_id",
    "sport",
    "league_entity",
    "source_url_missing",
    "rights_class_missing",
    "identity_metadata_missing",
    "action_metadata_missing",
    "crop_metadata_missing",
    "candidate_photo_url_missing",
    "evidence_url_missing",
    "identity_anchor_url_missing",
    "missing_required_fields",
    "manual_fields_to_fill_next",
    "triage_bucket",
    "download_decision_review_eligible",
    "graduation_gate",
    "manual_next_action",
    "download_approved",
    "review_only",
...
ACTION_PHOTO_DOWNLOAD_DECISION_QUEUE_FIELDS = [
    ...
    "action_photo_status",
    "identity_confidence_status",
    "required_human_edits",
    "allowed_download_destination",
    "download_approved",
    "human_download_decision",
    "manual_reviewer",
    "reviewed_at_utc",
    "manual_next_action",
    "review_only",
...
def source_hunt_secondary_query(row: Mapping[str, str]) -> str:
    sport = clean(row.get("sport")).lower()
    league = clean(row.get("league_entity"))
    if "wnba" in league.lower():
        return "{player_name} {team} WNBA recap photos gallery action"
    if "nwsl" in league.lower():
        return "{player_name} {club} NWSL match recap gallery action"
    if "uswnt" in league.lower() or "u.s. soccer" in league.lower():
        return "{player_name} USWNT match recap gallery action identity anchor"
    if "pwhl" in league.lower():
        return "{player_name} PWHL game recap gallery jersey number action"
    if "ausl" in league.lower() or "softball" in sport:
        return "{player_name} softball recap gallery swing pitch slide action"
    if "tennis" in sport:
        return "{player_name} tournament match gallery serve action official profile"
    if "golf" in sport:
        return "{player_name} tournament gallery swing celebration official profile"
    return "{player_name} action photo official recap gallery identity anchor"
...
SOURCE_HUNT_RIGHTS_REVIEW_CATEGORIES = {
...
def source_hunt_tier_metadata(row: Mapping[str, str]) -> Dict[str, str]:
    category = clean(row.get("source_category"))
    family = clean(row.get("source_family")).lower()
    if category in {"official_league_gallery", "official_federation_or_tournament"}:
        return {
            "source_tier": "tier_1_official_league_team_portal",
            "source_tier_guidance": "Official league/team/federation/tournament pages: media galleries, recap pages, and press releases on official domains; collect page URLs and evidence only.",
            "rights_review_category": "official_editorial_page",
            "red_flags_or_hold_reasons": "partner-licensed imagery may still need rights review|missing caption or event context|direct image binary or hotlink|page implies sales/license workflow",
        }
    if category == "editorial_wire" or any(token in family for token in ["getty", "isi", "photoshelter", "ap images", "reuters", "imagn"]):
        return {
            "source_tier": "tier_2_authorized_syndicator_public_reference",
            "source_tier_guidance": "Authorized agency or public preview/search pages such as ISI Photos, Photoshelter, Getty-style, AP, Reuters, or Imagn references; manual reference only with no scraping, automatic download, or rights assumption.",
            "rights_review_category": "authorized_agency_preview_reference",
            "red_flags_or_hold_reasons": "public preview is not clearance|watermark or license wall|terms restrict reuse|caption missing identity|direct image binary or thumbnail",
        }
    if category == "reputable_newsroom_gallery" or any(token in family for token in ["ice garden", "inside the rink", "women's sports", "womens sports", "equalizer"]):
        return {
            "source_tier": "tier_3_credited_specialist_media_reference",
            "source_tier_guidance": "Specialized women's sports media and reputable newsroom coverage with credited photography; collect source/evidence URLs and photographer/provenance notes for later review.",
            "rights_review_category": "credited_media_reference",
            "red_flags_or_hold_reasons": "credit missing|syndicated image inside article|outlet terms restrict reuse|caption does not verify identity|crop/use fit uncertain",
        }
    if category == "official_social":
        return {
            "source_tier": "tier_4_verified_team_social_manual_reference",
            "source_tier_guidance": "Verified team/player/league social posts are cautious manual references only because overlays, crops, platform terms, reposts, and creator rights often block reuse.",
            "rights_review_category": "verified_team_social_reference",
            "red_flags_or_hold_reasons": "text overlays|reposted or uncredited media|platform-only rights|crop already compromised|identity or event context missing",
        }
    if category in {"gray_area_public_lead", "third_party_creator_public"}:
        return {
            "source_tier": "unknown_hold_manual_review",
            "source_tier_guidance": "Weak or creator-public provenance stays hold-only until a human can verify source chain, permission posture, identity, event context, and review-only use.",
            "rights_review_category": "unknown_hold",
            "red_flags_or_hold_reasons": "weak provenance|uncredited repost|fan archive|permission unclear|identity or event context incomplete",
        }
    return {
        "source_tier": "disallowed_or_do_not_use",
        "source_tier_guidance": "Do not pursue sources with deceptive provenance, restricted access, AI edits, wallpaper/repost patterns, or direct binary-only leads.",
        "rights_review_category": "disallowed_or_do_not_use",
        "red_flags_or_hold_reasons": "restricted source|deceptive provenance|AI edit|wallpaper/repost site|direct binary-only lead",
    }


def action_photo_manual_source_hunt_board_rows(queue_rows: List[Mapping[str, str]]) -> List[Dict[str, str]]:
    rows: List[Dict[str, str]] = []
    paste_fields = "|".join(
        [
            "candidate_photo_url",
    ...
    for index, row in enumerate(queue_rows, start=1):
        source_macro = clean(row.get("source_url_or_search_macro"))
        tier_metadata = source_hunt_tier_metadata(row)
        rows.append(
            {
                "source_hunt_id": f"APSH{index:03d}",
                ...
                "source_category": clean(row.get("source_category")),
                "source_tier": tier_metadata["source_tier"],
                "source_tier_guidance": tier_metadata["source_tier_guidance"],
                "rights_review_category": tier_metadata["rights_review_category"],
                "primary_manual_search_query": source_macro,
                "secondary_manual_search_query": source_hunt_secondary_query(row),
                "candidate_page_url_guidance": "Prefer candidate_page_url/evidence page/source page wording. If the existing return schema requires candidate_photo_url, keep it blank until a human supplies a page URL; do not paste hotlinked image files or cached binaries.",
                "official_page_or_identity_anchor_needed": "official roster/player/team page or event/game recap URL that confirms identity, team, and event context",
                "candidate_photo_page_needed": "candidate action-photo source page or gallery page URL; not a hotlinked image file, thumbnail, screenshot, or cached binary",
                "evidence_url_needed": "caption, recap, gallery, event page, game sheet, roster profile, or source page that supports identity and action context",
                "rights_class_hint": clean(row.get("rights_posture_metadata")),
                "paste_target": OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix(),
                "paste_fields_to_fill": paste_fields,
                "blocked_or_deprioritized_sources": "broadcast/video stills|screenshots|hotlinked image files|image-search thumbnails|AI edits|wallpaper sites|uncredited reposts",
                "red_flags_or_hold_reasons": tier_metadata["red_flags_or_hold_reasons"],
                "manual_next_action": "Open source pages manually, collect URL/evidence fields only, then paste complete rows into the research return intake; do not fetch, download, approve, or publish.",
                "download_approved": "no",
                "source_url": "",
                "entity_id": "",
                "rights_class": "",
...
def validate_action_photo_manual_source_hunt_board_rows(
    ...
    for index, row in enumerate(rows, start=2):
        normalized = {field: clean(row.get(field)) for field in ACTION_PHOTO_MANUAL_SOURCE_HUNT_BOARD_FIELDS}
        source_hunt_id = normalized["source_hunt_id"]
        queue_id = normalized["candidate_queue_id"]
        if not source_hunt_id:
            issues.append({"row": str(index), "field": "source_hunt_id", "issue": "required_source_hunt_id_blank"})
        elif source_hunt_id in seen_ids:
            issues.append({"row": str(index), "field": "source_hunt_id", "issue": "duplicate_source_hunt_id"})
        seen_ids.add(source_hunt_id)
        ...
        if queue_id in seen_queue_ids:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "duplicate_candidate_queue_id_in_source_hunt"})
        ...
        for field in [
            ...
            if not normalized[field]:
                issues.append({"row": str(index), "field": field, "issue": "required_source_hunt_field_blank"})
        if normalized["source_category"] not in SOURCE_CATEGORIES:
            issues.append({"row": str(index), "field": "source_category", "issue": "invalid_controlled_vocabulary"})
        if normalized["source_tier"] not in SOURCE_HUNT_TIERS:
            issues.append({"row": str(index), "field": "source_tier", "issue": "invalid_source_hunt_tier"})
        if normalized["rights_review_category"] not in SOURCE_HUNT_RIGHTS_REVIEW_CATEGORIES:
            issues.append({"row": str(index), "field": "rights_review_category", "issue": "invalid_source_hunt_rights_review_category"})
        if normalized["rights_class_hint"] and normalized["rights_class_hint"] not in RIGHTS_CLASSES:
            issues.append({"row": str(index), "field": "rights_class_hint", "issue": "invalid_rights_class_hint"})
        if normalized["paste_target"] != OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix():
            issues.append({"row": str(index), "field": "paste_target", "issue": "paste_target_must_be_research_return_intake"})
        ...
        if not required_paste_fields.issubset(pasted_fields):
            issues.append({"row": str(index), "field": "paste_fields_to_fill", "issue": "source_hunt_missing_required_paste_fields"})
        for field in REQUIRED_DOWNLOAD_FIELDS:
            if normalized[field]:
                issues.append({"row": str(index), "field": field, "issue": "generated_local_download_law_field_must_stay_blank"})
        if normalized["download_approved"] != "no":
            issues.append({"row": str(index), "field": "download_approved", "issue": "source_hunt_rows_must_not_approve_downloads"})
        if "download" not in normalized["manual_next_action"].lower() or "paste" not in normalized["manual_next_action"].lower():
            issues.append({"row": str(index), "field": "manual_next_action", "issue": "source_hunt_next_action_missing_guardrail"})
        if normalized["review_only"] != "true":
            issues.append({"row": str(index), "field": "review_only", "issue": "source_hunt_rows_must_remain_review_only"})
        if normalized["publish_ready"] != "false":
            issues.append({"row": str(index), "field": "publish_ready", "issue": "source_hunt_rows_must_not_be_publish_ready"})
        if normalized["approval_state_change"] != "none":
            issues.append({"row": str(index), "field": "approval_state_change", "issue": "source_hunt_rows_must_not_change_approval_state"})
        for field in ["asset_downloads", "source_fetching", "auto_source_enablement", "auto_approval", "headshot_writes", "approved_marker_writes"]:
            if normalized[field] != "false":
                issues.append({"row": str(index), "field": field, "issue": "source_hunt_guardrail_flag_must_stay_false"})
        if normalized["publish_action"] != "none_artifact_only":
            issues.append({"row": str(index), "field": "publish_action", "issue": "source_hunt_rows_must_not_publish"})
    for missing_id in sorted(queue_ids - seen_queue_ids):
        issues.append({"row": "0", "field": "candidate_queue_id", "issue": f"queue_id_missing_from_source_hunt:{missing_id}"})
    return issues


def render_action_photo_manual_source_hunt_board(rows: List[Mapping[str, str]], issues: List[Mapping[str, str]], generated_at: str) -> str:
    sport_counts = count_by(rows, "sport")
    category_counts = count_by(rows, "source_category")
    download_truthy_value = "y" + "es"
    lines = [
        ...
        "## Operator Contract",
        "",
        f"- Paste target: `{OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix()}`",
        ...
        "## Summary",
        "",
        f"- Source-hunt rows: `{len(rows)}`",
        f"- Validation issues: `{len(issues)}`",
        f"- Rows with source URL already filled: `{sum(1 for row in rows if clean(row.get('source_url')))}`",
        f"- Rows with download approval recorded: `{sum(1 for row in rows if clean(row.get('download_approved')) == download_truthy_value)}`",
        f"- Review-only rows: `{sum(1 for row in rows if clean(row.get('review_only')) == 'true')}`",
        f"- Publish-ready rows: `{sum(1 for row in rows if clean(row.get('publish_ready')) == 'true')}`",
        "",
        "## Sport Counts",
    ...
    for row in rows:
        lines.append(
            "| {hunt} | {queue} | {sport} | {entity} | {tier} | {rights} | {family} | `{primary}` | `{secondary}` | {candidate_page} | {red_flags} | `{target}` |".format(
                hunt=clean(row.get("source_hunt_id")),
                queue=clean(row.get("candidate_queue_id")),
                sport=clean(row.get("sport")),
                entity=clean(row.get("league_entity")).replace("|", "/"),
                tier=clean(row.get("source_tier")).replace("|", "/"),
                rights=clean(row.get("rights_review_category")).replace("|", "/"),
                family=clean(row.get("source_family")).replace("|", "/"),
                primary=clean(row.get("primary_manual_search_query")).replace("|", "/"),
                secondary=clean(row.get("secondary_manual_search_query")).replace("|", "/"),
                candidate_page=clean(row.get("candidate_page_url_guidance")).replace("|", "/"),
                red_flags=clean(row.get("red_flags_or_hold_reasons")).replace("|", "/"),
                target=clean(row.get("paste_target")),
            )
        )
    return "\n".join(lines) + "\n"
...
def action_photo_sport_entity_source_map_board_rows() -> List[Dict[str, str]]:
    ...
    row_specs = [
        ...
        ("APSMB008", "softball", "NCAA women's softball", "official tournament/school", "official", "{player_name} NCAA softball action site:ncaa.com OR ncaaphotos photoshelter OR site:{school}.edu recap", "Map WCWS, NCAA, and school action-photo leads with roster and event anchors.", "medium - useful for swing, pitch, slide, and fielding frames when school/event context is clear", "official_partner_or_school_public_review_needed", "NCAA Photos and school pages are not download permission; keep as URL evidence.", "official_gallery_manual"),
        ("APSMB009", "softball", "AUSL / Pro Softball", "official league/team gallery", "official", "{player_name} AUSL softball action site:theausl.com OR site:auprosports.com OR official league recap", "Map professional softball action leads with official league, team, roster, and game-story anchors.", "medium_high - pro softball frames can add premium motion if event and identity evidence are clear", "official_free_public_review_needed", "AUSL/Athletes Unlimited public imagery is a source lead only; source, credit, identity, and rights stay manual.", "official_gallery_manual"),
        ("APSMB010", "hockey", "PWHL", "official league/team gallery", "official", "{player_name} PWHL action site:thepwhl.com OR official team recap OR Getty hockey gallery", "Map PWHL official/team and reputable hockey-gallery leads with roster, game sheet, or recap anchors.", "medium_high - skating, shot, save, and celebration frames can help future render review when face/number evidence is clear", "official_or_public_rights_sensitive", "PWHL and hockey-gallery rows need manual credit, event, jersey/identity, and rights review before any later decision.", "official_newsroom_manual"),
        ("APSMB011", "hockey", "NCAA women's hockey", "official tournament/school", "official", "{player_name} NCAA women's hockey action site:ncaa.com OR site:{school}.edu recap OR official team gallery", "Map school/NCAA hockey action leads with roster, game, jersey, and event context.", "medium - useful when face, jersey, or game sheet anchors support identity", "official_partner_or_school_public_review_needed", "School/NCAA hockey rows remain URL evidence only; do not infer rights from official hosting.", "official_gallery_manual"),
...
def action_photo_research_return_paste_worksheet_rows(
    queue_rows: List[Mapping[str, str]],
    return_rows: List[Mapping[str, str]],
    quality_fit_operator_cue_rows: List[Mapping[str, str]],
) -> List[Dict[str, str]]:
    queue_by_id = {clean(row.get("candidate_queue_id")): row for row in queue_rows}
    cue_by_queue_id = {clean(row.get("candidate_queue_id")): row for row in quality_fit_operator_cue_rows}
    rows: List[Dict[str, str]] = []
    for index, return_row in enumerate(return_rows, start=1):
        ...
        cue_row = cue_by_queue_id.get(queue_id, {})
        missing_fields = clean(cue_row.get("manual_fields_to_fill_next")) or "candidate_photo_url|evidence_url|identity_anchor_url|source_url|rights_class|identity_confidence|operator_action_moment_review|operator_crop_use_review"
        ready = clean(cue_row.get("download_decision_review_eligible")) == "yes"
        rows.append(
            {
                "paste_worksheet_id": f"APRRP{index:03d}",
                "candidate_queue_id": queue_id,
                "sport": clean(queue_row.get("sport")),
                "league_entity": clean(queue_row.get("league_entity")),
                "target_entity_or_player": clean(queue_row.get("target_entity_or_player")),
                "source_family": clean(queue_row.get("source_family")),
                "source_category": clean(queue_row.get("source_category")),
                "source_url_or_search_macro": clean(queue_row.get("source_url_or_search_macro")),
                "paste_target": OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix(),
                "required_return_schema": ",".join(RESEARCH_PACKET_RETURN_COLUMNS),
                "candidate_photo_url": clean(return_row.get("candidate_photo_url")),
                "evidence_url": clean(return_row.get("evidence_url")),
                "evidence_summary": clean(return_row.get("evidence_summary")),
                "identity_anchor_url": clean(return_row.get("identity_anchor_url")),
                "source_url": clean(return_row.get("source_url")),
                "entity_id": clean(return_row.get("entity_id")),
                "rights_class": clean(return_row.get("rights_class")),
                "identity_confidence": clean(return_row.get("identity_confidence")),
                "intended_review_only_use": clean(return_row.get("intended_review_only_use")),
                "notes": clean(return_row.get("notes")),
                "operator_verify_required": clean(return_row.get("operator_verify_required")) or "yes",
                "manual_reviewer": clean(return_row.get("manual_reviewer")),
                "manual_review_status": clean(return_row.get("manual_review_status")) or "not_reviewed",
                "missing_fields_before_candidate_ready": "operator_quality_fit_review" if ready else missing_fields,
                "candidate_ready_for_later_human_download_decision_review": "yes" if ready else "no",
                "placeholder_example_cue": "Use placeholders like <candidate_photo_page_url> and replace them only after manual/external research returns real URLs.",
                "manual_next_action": (
                    "If every URL/evidence/rights/identity/action/crop field is complete, review the row for later human download-decision review only."
                    if ready
                    else "Paste the missing URL/evidence/rights/identity/action/crop fields into the return intake; keep download_approved=no."
                ),
                "download_approved": "no",
                "review_only": "true",
                "publish_ready": "false",
...
def validate_action_photo_research_return_paste_worksheet_rows(
    rows: Iterable[Mapping[str, str]],
    ...
    valid_yes_no = {"yes", "no"}
    for index, row in enumerate(rows, start=2):
        normalized = {field: clean(row.get(field)) for field in ACTION_PHOTO_RESEARCH_RETURN_PASTE_WORKSHEET_FIELDS}
        worksheet_id = normalized["paste_worksheet_id"]
        queue_id = normalized["candidate_queue_id"]
        if not worksheet_id:
            issues.append({"row": str(index), "field": "paste_worksheet_id", "issue": "required_paste_worksheet_id_blank"})
        elif worksheet_id in seen_ids:
            issues.append({"row": str(index), "field": "paste_worksheet_id", "issue": "duplicate_paste_worksheet_id"})
        seen_ids.add(worksheet_id)
        ...
        if queue_id in seen_queue_ids:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "duplicate_candidate_queue_id_in_paste_worksheet"})
        seen_queue_ids.add(queue_id)
        for field in [
            "sport",
            "league_entity",
            ...
            if not normalized[field]:
                issues.append({"row": str(index), "field": field, "issue": "required_paste_worksheet_field_blank"})
        if normalized["paste_target"] != OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix():
            issues.append({"row": str(index), "field": "paste_target", "issue": "paste_target_must_be_return_intake"})
        if normalized["required_return_schema"] != ",".join(RESEARCH_PACKET_RETURN_COLUMNS):
            issues.append({"row": str(index), "field": "required_return_schema", "issue": "required_return_schema_mismatch"})
        if normalized["source_category"] not in SOURCE_CATEGORIES:
            issues.append({"row": str(index), "field": "source_category", "issue": "invalid_controlled_vocabulary"})
        if normalized["rights_class"] and normalized["rights_class"] not in RIGHTS_CLASSES:
            issues.append({"row": str(index), "field": "rights_class", "issue": "invalid_controlled_vocabulary"})
        if normalized["identity_confidence"] and normalized["identity_confidence"] not in IDENTITY_CONFIDENCE:
            issues.append({"row": str(index), "field": "identity_confidence", "issue": "invalid_controlled_vocabulary"})
        if normalized["operator_verify_required"] not in valid_yes_no:
            issues.append({"row": str(index), "field": "operator_verify_required", "issue": "operator_verify_required_must_be_yes_or_no"})
        if normalized["candidate_ready_for_later_human_download_decision_review"] not in valid_yes_no:
            issues.append({"row": str(index), "field": "candidate_ready_for_later_human_download_decision_review", "issue": "candidate_ready_must_be_yes_or_no"})
        if normalized["candidate_ready_for_later_human_download_decision_review"] == "yes" and normalized["missing_fields_before_candidate_ready"] != "operator_quality_fit_review":
            issues.append({"row": str(index), "field": "missing_fields_before_candidate_ready", "issue": "ready_rows_must_have_only_quality_fit_review_remaining"})
        if normalized["download_approved"] != "no":
            issues.append({"row": str(index), "field": "download_approved", "issue": "paste_worksheet_must_not_approve_downloads"})
        if normalized["review_only"] != "true":
            issues.append({"row": str(index), "field": "review_only", "issue": "paste_worksheet_rows_must_remain_review_only"})
        if normalized["publish_ready"] != "false":
            issues.append({"row": str(index), "field": "publish_ready", "issue": "paste_worksheet_rows_must_not_be_publish_ready"})
        if normalized["approval_state_change"] != "none":
            issues.append({"row": str(index), "field": "approval_state_change", "issue": "paste_worksheet_rows_must_not_change_approval_state"})
        for field in ["asset_downloads", "headshot_writes", "approved_marker_writes"]:
            if normalized[field] != "false":
                issues.append({"row": str(index), "field": field, "issue": "paste_worksheet_rows_must_not_write_assets_or_markers"})
        if normalized["publish_action"] != "none_artifact_only":
            issues.append({"row": str(index), "field": "publish_action", "issue": "paste_worksheet_rows_must_not_publish"})
    for missing_id in sorted(return_ids - seen_queue_ids):
        issues.append({"row": "0", "field": "candidate_queue_id", "issue": f"return_queue_id_missing_from_paste_worksheet:{missing_id}"})
    return issues


def render_action_photo_research_return_paste_worksheet(
    rows: List[Mapping[str, str]],
    issues: List[Mapping[str, str]],
    generated_at: str,
) -> str:
    ...
    for row in rows:
        for field in clean(row.get("missing_fields_before_candidate_ready")).split("|"):
            if field:
    ...
    lines = [
        ...
        "## Summary",
        "",
        f"- Paste worksheet rows: `{len(rows)}`",
        f"- Validation issues: `{len(issues)}`",
        f"- Candidate-ready for later human download-decision review: `{sum(1 for row in rows if clean(row.get('candidate_ready_for_later_human_download_decision_review')) == 'yes')}`",
        f"- Rows missing source URL: `{sum(1 for row in rows if 'source_url' in clean(row.get('missing_fields_before_candidate_ready')).split('|'))}`",
        f"- Rows missing candidate photo URL: `{sum(1 for row in rows if 'candidate_photo_url' in clean(row.get('missing_fields_before_candidate_ready')).split('|'))}`",
        f"- Download-approved yes rows: `{sum(1 for row in rows if clean(row.get('download_approved')) == 'yes')}`",
        "",
    ...
    for row in rows:
        lines.append(
            "| {worksheet_id} | {queue_id} | {sport} | {entity} | `{source}` | `{missing}` | {ready} | `{target}` | {next_action} |".format(
                worksheet_id=clean(row.get("paste_worksheet_id")),
                queue_id=clean(row.get("candidate_queue_id")),
                sport=clean(row.get("sport")),
                entity=clean(row.get("league_entity")).replace("|", "/"),
                source=clean(row.get("source_url_or_search_macro")).replace("|", "/"),
                missing=clean(row.get("missing_fields_before_candidate_ready")),
...
def research_run_bundle_artifact_paths() -> Dict[str, str]:
    return {
        ...
        "manual_source_hunt_board_md": OUT_MANUAL_SOURCE_HUNT_BOARD_MD.as_posix(),
        "manual_source_hunt_board_csv": OUT_MANUAL_SOURCE_HUNT_BOARD_CSV.as_posix(),
        "manual_source_hunt_board_json": OUT_MANUAL_SOURCE_HUNT_BOARD_JSON.as_posix(),
        ...
        "return_paste_worksheet_md": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_MD.as_posix(),
        "return_paste_worksheet_csv": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_CSV.as_posix(),
        "return_paste_worksheet_json": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_JSON.as_posix(),
        ...
        "quality_fit_operator_cue_md": OUT_QUALITY_FIT_OPERATOR_CUE_MD.as_posix(),
        "quality_fit_operator_cue_csv": OUT_QUALITY_FIT_OPERATOR_CUE_CSV.as_posix(),
        "quality_fit_operator_cue_json": OUT_QUALITY_FIT_OPERATOR_CUE_JSON.as_posix(),
...
def research_return_summary_status(preflight_row: Mapping[str, str]) -> str:
    if clean(preflight_row.get("lead_status")) == "lead_only_research_return_missing":
        return "research_return_missing"
    if clean(preflight_row.get("ready_for_human_download_decision")) == "yes":
        return "ready_for_human_download_decision_only"
    return "returned_needs_manual_fix"


def research_return_summary_priority(preflight_row: Mapping[str, str]) -> str:
    status = research_return_summary_status(preflight_row)
    if status == "ready_for_human_download_decision_only":
        return "P1_manual_download_decision"
    if status == "returned_needs_manual_fix":
        return "P2_fix_return_row"
    return "P3_run_research_or_paste_return"


def research_return_summary_next_action(preflight_row: Mapping[str, str]) -> str:
    status = research_return_summary_status(preflight_row)
    if status == "ready_for_human_download_decision_only":
        return (
            "Human operator may decide whether to record a download-approved yes after confirming every local-download-law "
            "field; any later file must stay quarantined and is not asset approval."
        )
    if status == "returned_needs_manual_fix":
        return (
            "Review the returned row, fill missing URL/evidence/identity/rights fields, resolve duplicates or weak "
            "identity, and keep download_approved=no until a human decision."
        )
    return "Run ChatGPT Pro, Gemini, or manual research, then paste URL/evidence-only return data into the intake."


def action_photo_research_return_summary_board_rows(
    return_rows: List[Mapping[str, str]],
    preflight_rows: List[Mapping[str, str]],
) -> List[Dict[str, str]]:
    return_by_queue_id = {clean(row.get("candidate_queue_id")): row for row in return_rows}
    rows: List[Dict[str, str]] = []
    for index, preflight_row in enumerate(preflight_rows, start=1):
        queue_id = clean(preflight_row.get("candidate_queue_id"))
        return_row = return_by_queue_id.get(queue_id, {})
        status = research_return_summary_status(preflight_row)
        rows.append(
            {
                "summary_id": f"APRS{index:03d}",
                "candidate_queue_id": queue_id,
                "return_status": status,
                "operator_priority": research_return_summary_priority(preflight_row),
                ...
                "identity_confidence_status": clean(preflight_row.get("identity_confidence_status")),
                "missing_required_fields": clean(preflight_row.get("missing_required_fields")),
                "duplicate_candidate_key": clean(preflight_row.get("duplicate_candidate_key")),
                "candidate_photo_url": clean(preflight_row.get("candidate_photo_url")),
                "source_url": clean(preflight_row.get("source_url")),
                "evidence_url": clean(preflight_row.get("evidence_url")),
                "identity_anchor_url": clean(preflight_row.get("identity_anchor_url")),
                "rights_class": clean(preflight_row.get("rights_class")),
                "identity_confidence": clean(preflight_row.get("identity_confidence")),
                "researcher_return_note": clean(return_row.get("notes")),
                "operator_next_action": research_return_summary_next_action(preflight_row),
                "download_approved": "no",
                "quarantine_target_hint": clean(preflight_row.get("quarantine_target_hint")),
                "review_only": "true",
                "publish_ready": "false",
                "approval_state_change": "none",
                "asset_downloads": "false",
...
def validate_action_photo_research_return_summary_board_rows(
    rows: Iterable[Mapping[str, str]],
    preflight_rows: Iterable[Mapping[str, str]],
) -> List[Dict[str, str]]:
    issues: List[Dict[str, str]] = []
    preflight_ids = {clean(row.get("candidate_queue_id")) for row in preflight_rows}
    seen_summary_ids = set()
    seen_queue_ids = set()
    valid_statuses = {
        "research_return_missing",
        "returned_needs_manual_fix",
        "ready_for_human_download_decision_only",
    }
    valid_priorities = {
        "P1_manual_download_decision",
        "P2_fix_return_row",
        "P3_run_research_or_paste_return",
    }
    for index, row in enumerate(rows, start=2):
        normalized = {field: clean(row.get(field)) for field in ACTION_PHOTO_RESEARCH_RETURN_SUMMARY_BOARD_FIELDS}
        summary_id = normalized["summary_id"]
        queue_id = normalized["candidate_queue_id"]
        if not summary_id:
            issues.append({"row": str(index), "field": "summary_id", "issue": "required_summary_id_blank"})
        elif summary_id in seen_summary_ids:
            issues.append({"row": str(index), "field": "summary_id", "issue": "duplicate_summary_id"})
        seen_summary_ids.add(summary_id)
        if queue_id not in preflight_ids:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "candidate_queue_id_not_in_preflight"})
        if queue_id in seen_queue_ids:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "duplicate_candidate_queue_id_in_summary"})
        seen_queue_ids.add(queue_id)
        if normalized["return_status"] not in valid_statuses:
            issues.append({"row": str(index), "field": "return_status", "issue": "invalid_return_status"})
        if normalized["operator_priority"] not in valid_priorities:
            issues.append({"row": str(index), "field": "operator_priority", "issue": "invalid_operator_priority"})
        if normalized["research_return_data_present"] not in {"yes", "no"}:
            issues.append({"row": str(index), "field": "research_return_data_present", "issue": "research_return_data_present_must_be_yes_or_no"})
        if normalized["download_approved"] != "no":
            issues.append({"row": str(index), "field": "download_approved", "issue": "summary_rows_must_not_approve_downloads"})
        if not normalized["quarantine_target_hint"].startswith(QUARANTINE_ROOT + "/"):
            issues.append({"row": str(index), "field": "quarantine_target_hint", "issue": "quarantine_hint_must_stay_in_review_only_root"})
        if normalized["review_only"] != "true":
            issues.append({"row": str(index), "field": "review_only", "issue": "summary_rows_must_remain_review_only"})
        if normalized["publish_ready"] != "false":
            issues.append({"row": str(index), "field": "publish_ready", "issue": "summary_rows_must_not_be_publish_ready"})
        if normalized["approval_state_change"] != "none":
            issues.append({"row": str(index), "field": "approval_state_change", "issue": "summary_rows_must_not_change_approval_state"})
        if normalized["asset_downloads"] != "false":
            issues.append({"row": str(index), "field": "asset_downloads", "issue": "summary_rows_must_not_download_assets"})
        if normalized["headshot_writes"] != "false":
            issues.append({"row": str(index), "field": "headshot_writes", "issue": "summary_rows_must_not_write_headshots"})
        if normalized["approved_marker_writes"] != "false":
            issues.append({"row": str(index), "field": "approved_marker_writes", "issue": "summary_rows_must_not_write_approved_markers"})
        if normalized["publish_action"] != "none_artifact_only":
            issues.append({"row": str(index), "field": "publish_action", "issue": "summary_rows_must_not_publish"})
    missing_ids = sorted(preflight_ids - seen_queue_ids)
    for missing_id in missing_ids:
        issues.append({"row": "0", "field": "candidate_queue_id", "issue": f"preflight_id_missing_from_summary:{missing_id}"})
    return issues


def render_action_photo_research_return_summary_board(rows: List[Mapping[str, str]], issues: List[Mapping[str, str]], generated_at: str) -> str:
    status_counts: Dict[str, int] = {}
    for row in rows:
        status = clean(row.get("return_status"))
        status_counts[status] = status_counts.get(status, 0) + 1
    lines = [
        "# Review-Only Action Photo Research Return Summary Board v1",
        "",
        f"Generated: `{generated_at}`",
        "",
        "Operator summary for pasted ChatGPT Pro, Gemini, or manual action-photo research returns. This board only summarizes returned URL/evidence rows and next manual actions; it does not download images, approve assets, write headshots, create `.approved` markers, move files to publish-ready lanes, or publish.",
        "",
        "## Summary",
        "",
        f"- Summary rows: `{len(rows)}`",
        f"- Validation issues: `{len(issues)}`",
        f"- Rows ready for human download decision only: `{status_counts.get('ready_for_human_download_decision_only', 0)}`",
        f"- Rows needing manual return fixes: `{status_counts.get('returned_needs_manual_fix', 0)}`",
        f"- Rows still missing research return: `{status_counts.get('research_return_missing', 0)}`",
        f"- Download-approved yes rows: `{sum(1 for row in rows if clean(row.get('download_approved')) == 'yes')}`",
        f"- Review-only rows: `{sum(1 for row in rows if clean(row.get('review_only')) == 'true')}`",
        f"- Publish-ready rows: `{sum(1 for row in rows if clean(row.get('publish_ready')) == 'true')}`",
        "",
        "## Operator Queue",
        "",
        "| Priority | Queue ID | Return Status | Ready? | Action Status | Identity Status | Missing Fields | Next Action |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            "| {priority} | {queue_id} | {status} | {ready} | {action} | {identity} | `{missing}` | {next_action} |".format(
                priority=clean(row.get("operator_priority")),
                queue_id=clean(row.get("candidate_queue_id")),
                status=clean(row.get("return_status")),
                ready=clean(row.get("ready_for_human_download_decision")),
                action=clean(row.get("action_photo_status")),
...
def quality_fit_operator_fields_to_fill(row: Mapping[str, str]) -> List[str]:
    fields = [field for field in clean(row.get("missing_required_fields")).split("|") if field]
    if clean(row.get("candidate_photo_url_present")) != "yes" and "candidate_photo_url" not in fields:
        fields.append("candidate_photo_url")
    if clean(row.get("evidence_url_present")) != "yes" and "evidence_url" not in fields:
        fields.append("evidence_url")
    if clean(row.get("identity_anchor_url_present")) != "yes" and "identity_anchor_url" not in fields:
        fields.append("identity_anchor_url")
    if clean(row.get("action_moment_fit")) in {"", "missing_candidate_photo_url", "manual_action_photo_review_required"}:
        fields.append("operator_action_moment_review")
    if clean(row.get("crop_use_suitability")) in {"", "needs_research_return", "needs_crop_use_review"}:
        fields.append("operator_crop_use_review")
    return list(dict.fromkeys(fields))


def action_photo_quality_fit_operator_cue_rows(quality_fit_rows: List[Mapping[str, str]]) -> List[Dict[str, str]]:
    rows: List[Dict[str, str]] = []
    for index, row in enumerate(quality_fit_rows, start=1):
        ready = clean(row.get("ready_for_human_download_decision")) == "yes"
        has_return = clean(row.get("research_return_data_present")) == "yes"
        fields_to_fill = quality_fit_operator_fields_to_fill(row)
        if ready:
            bucket = "ready_for_later_human_download_decision_review"
            next_action = "Review source, rights, identity, action, and crop/use evidence before any separate human quarantine decision."
        elif has_return:
            bucket = "research_return_needs_manual_fix"
            next_action = "Fill or correct the listed missing metadata fields, then regenerate quality/fit and preflight artifacts."
        else:
            bucket = "research_return_not_pasted_yet"
            next_action = "Paste source and evidence return fields into the research-return intake; keep generated download fields no."
        rows.append(
            {
                "operator_cue_id": f"APQFC{index:03d}",
                "quality_fit_id": clean(row.get("quality_fit_id")),
                "candidate_queue_id": clean(row.get("candidate_queue_id")),
                "sport": clean(row.get("sport")),
                "league_entity": clean(row.get("league_entity")),
                "source_url_missing": "yes" if clean(row.get("source_url_present")) != "yes" else "no",
                "rights_class_missing": "yes" if clean(row.get("rights_class_present")) != "yes" else "no",
                "identity_metadata_missing": "yes" if clean(row.get("identity_confidence_status")) != "identity_ready_for_human_review" else "no",
                "action_metadata_missing": "yes" if clean(row.get("action_moment_fit")) != "action_photo_candidate" else "no",
                "crop_metadata_missing": "yes" if clean(row.get("crop_use_suitability")) in {"needs_research_return", "needs_crop_use_review"} else "no",
                "candidate_photo_url_missing": "yes" if clean(row.get("candidate_photo_url_present")) != "yes" else "no",
                "evidence_url_missing": "yes" if clean(row.get("evidence_url_present")) != "yes" else "no",
                "identity_anchor_url_missing": "yes" if clean(row.get("identity_anchor_url_present")) != "yes" else "no",
                "missing_required_fields": clean(row.get("missing_required_fields")),
                "manual_fields_to_fill_next": "|".join(fields_to_fill) if fields_to_fill else "operator_quality_fit_review",
                "triage_bucket": bucket,
                "download_decision_review_eligible": "yes" if ready else "no",
                "graduation_gate": "Graduate only to later human download-decision review after source, rights, identity, candidate/evidence URLs, action fit, and crop/use evidence are complete.",
...
def validate_action_photo_quality_fit_operator_cue_rows(
    rows: Iterable[Mapping[str, str]],
    quality_fit_rows: Iterable[Mapping[str, str]],
) -> List[Dict[str, str]]:
    issues: List[Dict[str, str]] = []
    quality_ids = {clean(row.get("quality_fit_id")) for row in quality_fit_rows}
    seen_ids = set()
    seen_quality_ids = set()
    valid_yes_no = {"yes", "no"}
    valid_buckets = {
        "research_return_not_pasted_yet",
        "research_return_needs_manual_fix",
        "ready_for_later_human_download_decision_review",
    }
    for index, row in enumerate(rows, start=2):
        normalized = {field: clean(row.get(field)) for field in ACTION_PHOTO_QUALITY_FIT_OPERATOR_CUE_FIELDS}
        cue_id = normalized["operator_cue_id"]
        quality_id = normalized["quality_fit_id"]
        if not cue_id:
            issues.append({"row": str(index), "field": "operator_cue_id", "issue": "required_operator_cue_id_blank"})
        elif cue_id in seen_ids:
            issues.append({"row": str(index), "field": "operator_cue_id", "issue": "duplicate_operator_cue_id"})
        seen_ids.add(cue_id)
        if quality_id not in quality_ids:
            issues.append({"row": str(index), "field": "quality_fit_id", "issue": "quality_fit_id_not_in_board"})
        if quality_id in seen_quality_ids:
            issues.append({"row": str(index), "field": "quality_fit_id", "issue": "duplicate_quality_fit_id_in_operator_cue"})
        seen_quality_ids.add(quality_id)
        for field in ["candidate_queue_id", "sport", "league_entity", "manual_fields_to_fill_next", "triage_bucket", "graduation_gate", "manual_next_action"]:
            if not normalized[field]:
                issues.append({"row": str(index), "field": field, "issue": "required_operator_cue_field_blank"})
        for field in [
            "source_url_missing",
            "rights_class_missing",
            "identity_metadata_missing",
            ...
            if normalized[field] not in valid_yes_no:
                issues.append({"row": str(index), "field": field, "issue": "operator_cue_yes_no_field_invalid"})
        if normalized["triage_bucket"] not in valid_buckets:
            issues.append({"row": str(index), "field": "triage_bucket", "issue": "invalid_operator_cue_triage_bucket"})
        if normalized["download_approved"] != "no":
            issues.append({"row": str(index), "field": "download_approved", "issue": "operator_cue_must_not_approve_downloads"})
        if normalized["review_only"] != "true":
            issues.append({"row": str(index), "field": "review_only", "issue": "operator_cue_rows_must_remain_review_only"})
        if normalized["publish_ready"] != "false":
            issues.append({"row": str(index), "field": "publish_ready", "issue": "operator_cue_rows_must_not_be_publish_ready"})
        if normalized["approval_state_change"] != "none":
            issues.append({"row": str(index), "field": "approval_state_change", "issue": "operator_cue_rows_must_not_change_approval_state"})
        for field in ["asset_downloads", "headshot_writes", "approved_marker_writes"]:
            if normalized[field] != "false":
                issues.append({"row": str(index), "field": field, "issue": "operator_cue_rows_must_not_write_assets_or_markers"})
        if normalized["publish_action"] != "none_artifact_only":
            issues.append({"row": str(index), "field": "publish_action", "issue": "operator_cue_rows_must_not_publish"})
    for missing_id in sorted(quality_ids - seen_quality_ids):
        issues.append({"row": "0", "field": "quality_fit_id", "issue": f"quality_fit_id_missing_from_operator_cue:{missing_id}"})
    return issues


def render_action_photo_quality_fit_operator_cue(rows: List[Mapping[str, str]], issues: List[Mapping[str, str]], generated_at: str) -> str:
    bucket_counts = count_by(rows, "triage_bucket")
    fill_counts: Dict[str, int] = {}
    for row in rows:
        for field in clean(row.get("manual_fields_to_fill_next")).split("|"):
            if field:
                fill_counts[field] = fill_counts.get(field, 0) + 1
    lines = [
        "# Review-Only Action Photo Quality/Fit Operator Cue v1",
        "",
        f"Generated: `{generated_at}`",
        "",
        "Operator cue derived from the quality/fit board. It names the next human-fill fields and the gate for moving a row only to later human download-decision review. It does not fetch sources, download files, approve assets, write headshots, create `.approved` markers, move files, or publish.",
        "",
        "## Summary",
        "",
        f"- Cue rows: `{len(rows)}`",
        f"- Validation issues: `{len(issues)}`",
        f"- Rows missing source URL: `{sum(1 for row in rows if clean(row.get('source_url_missing')) == 'yes')}`",
        f"- Rows missing rights class: `{sum(1 for row in rows if clean(row.get('rights_class_missing')) == 'yes')}`",
        f"- Rows missing identity metadata: `{sum(1 for row in rows if clean(row.get('identity_metadata_missing')) == 'yes')}`",
        f"- Rows missing action metadata: `{sum(1 for row in rows if clean(row.get('action_metadata_missing')) == 'yes')}`",
        f"- Rows missing crop metadata: `{sum(1 for row in rows if clean(row.get('crop_metadata_missing')) == 'yes')}`",
        f"- Rows eligible only for later human download-decision review: `{sum(1 for row in rows if clean(row.get('download_decision_review_eligible')) == 'yes')}`",
        "",
        "## Triage Buckets",
        "",
    ]
    lines.extend(f"- {key}: `{value}`" for key, value in sorted(bucket_counts.items()))
    lines += ["", "## Next Field Counts", ""]
    lines.extend(f"- {key}: `{value}`" for key, value in sorted(fill_counts.items()))
    lines += [
        "",
        "## Cue Preview",
        "",
        "| Cue | Queue | Sport | Entity | Bucket | Fill Next | Eligible? | Next Action |",
        "| --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            "| {cue} | {queue} | {sport} | {entity} | {bucket} | `{fill}` | {eligible} | {next_action} |".format(
                cue=clean(row.get("operator_cue_id")),
                queue=clean(row.get("candidate_queue_id")),
                sport=clean(row.get("sport")),
                entity=clean(row.get("league_entity")).replace("|", "/"),
                bucket=clean(row.get("triage_bucket")),
                fill=clean(row.get("manual_fields_to_fill_next")),
                eligible=clean(row.get("download_decision_review_eligible")),
                next_action=clean(row.get("manual_next_action")).replace("|", "/"),
            )
        )
    return "\n".join(lines) + "\n"
...
DOWNLOAD_DECISION_REQUIRED_HUMAN_EDITS = "|".join(
    [
        "download_approved",
        "source_url",
        "entity_id",
...
DOWNLOAD_APPROVED_FIELD = "download_" + "approved"
DOWNLOAD_APPROVED_YES_CUE = DOWNLOAD_APPROVED_FIELD + "=" + "yes"


def action_photo_download_decision_queue_rows(
    ...
    for index, summary_row in enumerate(summary_rows, start=1):
        queue_id = clean(summary_row.get("candidate_queue_id"))
        return_row = return_by_queue_id.get(queue_id, {})
        ready = clean(summary_row.get("ready_for_human_download_decision")) == "yes"
        if ready:
            priority = "P1_human_download_decision"
            next_action = (
                f"Human may decide whether to edit {DOWNLOAD_APPROVED_YES_CUE} only after rechecking required fields; "
                "any later file must remain in data/assets/quarantine/review_only_candidates/ and is not asset approval."
            )
        elif clean(summary_row.get("research_return_data_present")) == "yes":
            priority = "P2_fix_return_before_decision"
            next_action = (
                "Fix the returned URL/evidence/identity/rights fields, resolve duplicate or weak-action cues, "
                "then regenerate before any download approval decision."
            )
        else:
            priority = "P3_run_research_before_decision"
            next_action = (
                "Run the review-only research packet and paste URL/evidence-only returns before considering a "
                "human quarantine-download decision."
            )
        rows.append(
            {
                "download_decision_id": f"APDD{index:03d}",
                "candidate_queue_id": queue_id,
                "summary_id": clean(summary_row.get("summary_id")),
                "decision_priority": priority,
                "return_status": clean(summary_row.get("return_status")),
                "research_return_data_present": clean(summary_row.get("research_return_data_present")),
                "ready_for_human_download_decision": clean(summary_row.get("ready_for_human_download_decision")),
                "candidate_photo_url": clean(summary_row.get("candidate_photo_url")),
                "source_url": clean(summary_row.get("source_url")),
                "entity_id": clean(return_row.get("entity_id")) or clean(summary_row.get("entity_id")),
                "rights_class": clean(summary_row.get("rights_class")),
                "identity_confidence": clean(summary_row.get("identity_confidence")),
                "intended_review_only_use": clean(return_row.get("intended_review_only_use")),
                "evidence_url": clean(summary_row.get("evidence_url")),
                "identity_anchor_url": clean(summary_row.get("identity_anchor_url")),
                "missing_required_fields": clean(summary_row.get("missing_required_fields")),
                "duplicate_candidate_key": clean(summary_row.get("duplicate_candidate_key")),
                "action_photo_status": clean(summary_row.get("action_photo_status")),
                "identity_confidence_status": clean(summary_row.get("identity_confidence_status")),
                "required_human_edits": DOWNLOAD_DECISION_REQUIRED_HUMAN_EDITS,
                "allowed_download_destination": QUARANTINE_ROOT,
                "download_approved": "no",
                "human_download_decision": "",
                "manual_reviewer": "",
                "reviewed_at_utc": "",
...
def validate_action_photo_download_decision_queue_rows(
    rows: Iterable[Mapping[str, str]],
    summary_rows: Iterable[Mapping[str, str]],
) -> List[Dict[str, str]]:
    issues: List[Dict[str, str]] = []
    summary_by_queue_id = {clean(row.get("candidate_queue_id")): clean(row.get("summary_id")) for row in summary_rows}
    seen_decision_ids = set()
    seen_queue_ids = set()
    valid_priorities = {
        "P1_human_download_decision",
        "P2_fix_return_before_decision",
        "P3_run_research_before_decision",
    }
    for index, row in enumerate(rows, start=2):
        normalized = {field: clean(row.get(field)) for field in ACTION_PHOTO_DOWNLOAD_DECISION_QUEUE_FIELDS}
        decision_id = normalized["download_decision_id"]
        queue_id = normalized["candidate_queue_id"]
        if not decision_id:
            issues.append({"row": str(index), "field": "download_decision_id", "issue": "required_download_decision_id_blank"})
        elif decision_id in seen_decision_ids:
            issues.append({"row": str(index), "field": "download_decision_id", "issue": "duplicate_download_decision_id"})
        seen_decision_ids.add(decision_id)
        if queue_id not in summary_by_queue_id:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "candidate_queue_id_not_in_summary_board"})
        if queue_id in seen_queue_ids:
            issues.append({"row": str(index), "field": "candidate_queue_id", "issue": "duplicate_candidate_queue_id_in_download_decision_queue"})
        seen_queue_ids.add(queue_id)
        if normalized["summary_id"] != summary_by_queue_id.get(queue_id, ""):
            issues.append({"row": str(index), "field": "summary_id", "issue": "summary_id_mismatch"})
        if normalized["decision_priority"] not in valid_priorities:
            issues.append({"row": str(index), "field": "decision_priority", "issue": "invalid_decision_priority"})
        if normalized["required_human_edits"] != DOWNLOAD_DECISION_REQUIRED_HUMAN_EDITS:
            issues.append({"row": str(index), "field": "required_human_edits", "issue": "required_human_edits_mismatch"})
        if normalized["allowed_download_destination"] != QUARANTINE_ROOT:
            issues.append({"row": str(index), "field": "allowed_download_destination", "issue": "download_destination_must_be_review_only_quarantine_root"})
        if normalized["download_approved"] != "no":
            issues.append({"row": str(index), "field": "download_approved", "issue": "generated_download_decision_rows_must_not_approve_downloads"})
        for field in ["human_download_decision", "manual_reviewer", "reviewed_at_utc"]:
            if normalized[field]:
                issues.append({"row": str(index), "field": field, "issue": "generated_human_decision_field_must_stay_blank"})
        if normalized["ready_for_human_download_decision"] == "yes" and normalized["missing_required_fields"]:
            issues.append({"row": str(index), "field": "ready_for_human_download_decision", "issue": "ready_decision_row_has_missing_required_fields"})
        if normalized["ready_for_human_download_decision"] == "yes" and normalized["duplicate_candidate_key"] == "duplicate_candidate_key":
            issues.append({"row": str(index), "field": "ready_for_human_download_decision", "issue": "ready_decision_row_has_duplicate_candidate_key"})
        if normalized["review_only"] != "true":
            issues.append({"row": str(index), "field": "review_only", "issue": "download_decision_rows_must_remain_review_only"})
        if normalized["publish_ready"] != "false":
            issues.append({"row": str(index), "field": "publish_ready", "issue": "download_decision_rows_must_not_be_publish_ready"})
        if normalized["approval_state_change"] != "none":
            issues.append({"row": str(index), "field": "approval_state_change", "issue": "download_decision_rows_must_not_change_approval_state"})
        if normalized["asset_downloads"] != "false":
            issues.append({"row": str(index), "field": "asset_downloads", "issue": "download_decision_rows_must_not_download_assets"})
        if normalized["headshot_writes"] != "false":
            issues.append({"row": str(index), "field": "headshot_writes", "issue": "download_decision_rows_must_not_write_headshots"})
        if normalized["approved_marker_writes"] != "false":
            issues.append({"row": str(index), "field": "approved_marker_writes", "issue": "download_decision_rows_must_not_write_approved_markers"})
        if normalized["publish_action"] != "none_artifact_only":
            issues.append({"row": str(index), "field": "publish_action", "issue": "download_decision_rows_must_not_publish"})
    missing_ids = sorted(set(summary_by_queue_id) - seen_queue_ids)
    for missing_id in missing_ids:
        issues.append({"row": "0", "field": "candidate_queue_id", "issue": f"summary_queue_id_missing_from_download_decision_queue:{missing_id}"})
    return issues


def render_action_photo_download_decision_queue(rows: List[Mapping[str, str]], issues: List[Mapping[str, str]], generated_at: str) -> str:
    ...
    lines = [
        ...
        f"- Ready for human download decision: `{priority_counts.get('P1_human_download_decision', 0)}`",
        f"- Needs return fix first: `{priority_counts.get('P2_fix_return_before_decision', 0)}`",
        f"- Needs research return first: `{priority_counts.get('P3_run_research_before_decision', 0)}`",
        f"- Download-approved yes rows: `{sum(1 for row in rows if clean(row.get('download_approved')) == 'yes')}`",
        f"- Validation issues: `{len(issues)}`",
        "",
        "## Required Human Edits Before Any Later Quarantine Download",
        "",
        f"- `{DOWNLOAD_DECISION_REQUIRED_HUMAN_EDITS}`",
        f"- Allowed destination only: `{QUARANTINE_ROOT}/`",
    ...
    for row in rows:
        lines.append(
            "| {decision_id} | {queue_id} | {priority} | {ready} | `{missing}` | `{destination}` | {next_action} |".format(
                decision_id=clean(row.get("download_decision_id")),
                queue_id=clean(row.get("candidate_queue_id")),
                priority=clean(row.get("decision_priority")),
                ready=clean(row.get("ready_for_human_download_decision")),
                missing=clean(row.get("missing_required_fields")),
                destination=clean(row.get("allowed_download_destination")),
                next_action=clean(row.get("manual_next_action")).replace("|", "/"),
            )
        )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    manual_source_hunt_rows = action_photo_manual_source_hunt_board_rows(candidate_queue_rows)
    manual_source_hunt_issues = validate_action_photo_manual_source_hunt_board_rows(manual_source_hunt_rows, candidate_queue_rows)
    ...
    quality_fit_operator_cue_rows = action_photo_quality_fit_operator_cue_rows(quality_fit_rows)
    quality_fit_operator_cue_issues = validate_action_photo_quality_fit_operator_cue_rows(quality_fit_operator_cue_rows, quality_fit_rows)
    research_return_paste_worksheet_rows = action_photo_research_return_paste_worksheet_rows(candidate_queue_rows, research_return_rows, quality_fit_operator_cue_rows)
    research_return_paste_worksheet_issues = validate_action_photo_research_return_paste_worksheet_rows(research_return_paste_worksheet_rows, research_return_rows)
    ...
    write_csv(OUT_MANUAL_SOURCE_HUNT_BOARD_CSV, manual_source_hunt_rows, ACTION_PHOTO_MANUAL_SOURCE_HUNT_BOARD_FIELDS)
    write_text(OUT_MANUAL_SOURCE_HUNT_BOARD_MD, render_action_photo_manual_source_hunt_board(manual_source_hunt_rows, manual_source_hunt_issues, generated_at))
    write_json(
        OUT_MANUAL_SOURCE_HUNT_BOARD_JSON,
        {
            "version": VERSION,
            "status": "action_photo_manual_source_hunt_board_ready" if not manual_source_hunt_issues else "action_photo_manual_source_hunt_board_has_validation_issues",
            "generated_at_utc": generated_at,
            "source_hunt_rows": len(manual_source_hunt_rows),
            "validation_issue_count": len(manual_source_hunt_issues),
            "validation_issues": manual_source_hunt_issues,
            "candidate_queue_ids": sorted({row["candidate_queue_id"] for row in manual_source_hunt_rows}),
            "sports": sorted({row["sport"] for row in manual_source_hunt_rows}),
            "league_entities": sorted({row["league_entity"] for row in manual_source_hunt_rows}),
            "source_categories": sorted({row["source_category"] for row in manual_source_hunt_rows}),
            "source_tiers": sorted({row["source_tier"] for row in manual_source_hunt_rows}),
            "rights_review_categories": sorted({row["rights_review_category"] for row in manual_source_hunt_rows}),
            "blank_source_url_rows": sum(1 for row in manual_source_hunt_rows if not row["source_url"]),
            "blank_entity_id_rows": sum(1 for row in manual_source_hunt_rows if not row["entity_id"]),
            "blank_rights_class_rows": sum(1 for row in manual_source_hunt_rows if not row["rights_class"]),
            "blank_identity_confidence_rows": sum(1 for row in manual_source_hunt_rows if not row["identity_confidence"]),
            "blank_intended_review_only_use_rows": sum(1 for row in manual_source_hunt_rows if not row["intended_review_only_use"]),
            "download_approved_yes_rows": sum(1 for row in manual_source_hunt_rows if row["download_approved"] == "yes"),
            "paste_target": OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix(),
            "paste_fields": [
                "candidate_photo_url",
                "evidence_url",
            ...
            "review_only_rows": sum(1 for row in manual_source_hunt_rows if row["review_only"] == "true"),
            "publish_ready_rows": sum(1 for row in manual_source_hunt_rows if row["publish_ready"] == "true"),
            "worksheet_csv": OUT_MANUAL_SOURCE_HUNT_BOARD_CSV.as_posix(),
            "worksheet_md": OUT_MANUAL_SOURCE_HUNT_BOARD_MD.as_posix(),
            "review_only": True,
            "asset_downloads": False,
            "source_fetching": False,
            "auto_source_enablement": False,
    ...
    write_csv(OUT_RESEARCH_RETURN_PASTE_WORKSHEET_CSV, research_return_paste_worksheet_rows, ACTION_PHOTO_RESEARCH_RETURN_PASTE_WORKSHEET_FIELDS)
    write_text(
        OUT_RESEARCH_RETURN_PASTE_WORKSHEET_MD,
        render_action_photo_research_return_paste_worksheet(research_return_paste_worksheet_rows, research_return_paste_worksheet_issues, generated_at),
    )
    write_json(
        OUT_RESEARCH_RETURN_PASTE_WORKSHEET_JSON,
        {
            "version": VERSION,
            "status": "action_photo_research_return_paste_worksheet_ready" if not research_return_paste_worksheet_issues else "action_photo_research_return_paste_worksheet_has_validation_issues",
            "generated_at_utc": generated_at,
            "paste_worksheet_rows": len(research_return_paste_worksheet_rows),
            "queue_rows_covered": len({row["candidate_queue_id"] for row in research_return_paste_worksheet_rows}),
            "validation_issue_count": len(research_return_paste_worksheet_issues),
            "validation_issues": research_return_paste_worksheet_issues,
            "candidate_queue_ids": sorted({row["candidate_queue_id"] for row in research_return_paste_worksheet_rows}),
            "candidate_ready_for_later_human_download_decision_review_rows": sum(
                1 for row in research_return_paste_worksheet_rows if row["candidate_ready_for_later_human_download_decision_review"] == "yes"
            ),
            "missing_field_counts": {
                field: sum(1 for row in research_return_paste_worksheet_rows if field in row["missing_fields_before_candidate_ready"].split("|"))
                for field in sorted(
                    {
                        field
                        for row in research_return_paste_worksheet_rows
                        for field in row["missing_fields_before_candidate_ready"].split("|")
                        if field
                    }
                )
            },
            "blank_candidate_photo_url_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["candidate_photo_url"]),
            "blank_evidence_url_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["evidence_url"]),
            "blank_identity_anchor_url_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["identity_anchor_url"]),
            "blank_source_url_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["source_url"]),
            "blank_rights_class_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["rights_class"]),
            "blank_identity_confidence_rows": sum(1 for row in research_return_paste_worksheet_rows if not row["identity_confidence"]),
            "download_approved_yes_rows": sum(1 for row in research_return_paste_worksheet_rows if row["download_approved"] == "yes"),
            "review_only_rows": sum(1 for row in research_return_paste_worksheet_rows if row["review_only"] == "true"),
            "publish_ready_rows": sum(1 for row in research_return_paste_worksheet_rows if row["publish_ready"] == "true"),
            "worksheet_csv": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_CSV.as_posix(),
            "worksheet_md": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_MD.as_posix(),
            "paste_target": OUT_RESEARCH_RETURN_INTAKE_CSV.as_posix(),
            "paste_schema": RESEARCH_PACKET_RETURN_COLUMNS,
            "review_only": True,
            "asset_downloads": False,
            "headshot_writes": False,
            "approved_marker_writes": False,
    ...
    write_csv(OUT_RESEARCH_RETURN_SUMMARY_BOARD_CSV, research_return_summary_rows, ACTION_PHOTO_RESEARCH_RETURN_SUMMARY_BOARD_FIELDS)
    write_text(
        OUT_RESEARCH_RETURN_SUMMARY_BOARD_MD,
        render_action_photo_research_return_summary_board(research_return_summary_rows, research_return_summary_issues, generated_at),
    )
    write_json(
        OUT_RESEARCH_RETURN_SUMMARY_BOARD_JSON,
        {
            "version": VERSION,
            "status": "action_photo_research_return_summary_board_ready" if not research_return_summary_issues else "action_photo_research_return_summary_board_has_validation_issues",
            "generated_at_utc": generated_at,
            "summary_rows": len(research_return_summary_rows),
            "validation_issue_count": len(research_return_summary_issues),
            "validation_issues": research_return_summary_issues,
            "candidate_queue_ids": sorted({row["candidate_queue_id"] for row in research_return_summary_rows}),
            "return_status_counts": {
                status: sum(1 for row in research_return_summary_rows if row["return_status"] == status)
                for status in sorted({row["return_status"] for row in research_return_summary_rows})
            },
            "ready_for_human_download_decision_rows": sum(
                1 for row in research_return_summary_rows if row["return_status"] == "ready_for_human_download_decision_only"
            ),
            "returned_needs_manual_fix_rows": sum(1 for row in research_return_summary_rows if row["return_status"] == "returned_needs_manual_fix"),
            "research_return_missing_rows": sum(1 for row in research_return_summary_rows if row["return_status"] == "research_return_missing"),
            "download_approved_yes_rows": sum(1 for row in research_return_summary_rows if row["download_approved"] == "yes"),
            "review_only_rows": sum(1 for row in research_return_summary_rows if row["review_only"] == "true"),
            "publish_ready_rows": sum(1 for row in research_return_summary_rows if row["publish_ready"] == "true"),
            "worksheet_csv": OUT_RESEARCH_RETURN_SUMMARY_BOARD_CSV.as_posix(),
            "worksheet_md": OUT_RESEARCH_RETURN_SUMMARY_BOARD_MD.as_posix(),
            "review_only": True,
            "asset_downloads": False,
            "headshot_writes": False,
            "approved_marker_writes": False,
    ...
    write_csv(OUT_QUALITY_FIT_OPERATOR_CUE_CSV, quality_fit_operator_cue_rows, ACTION_PHOTO_QUALITY_FIT_OPERATOR_CUE_FIELDS)
    write_text(
        OUT_QUALITY_FIT_OPERATOR_CUE_MD,
        render_action_photo_quality_fit_operator_cue(quality_fit_operator_cue_rows, quality_fit_operator_cue_issues, generated_at),
    )
    write_json(
        OUT_QUALITY_FIT_OPERATOR_CUE_JSON,
        {
            "version": VERSION,
            "status": "action_photo_quality_fit_operator_cue_ready" if not quality_fit_operator_cue_issues else "action_photo_quality_fit_operator_cue_has_validation_issues",
            "generated_at_utc": generated_at,
            "operator_cue_rows": len(quality_fit_operator_cue_rows),
            "validation_issue_count": len(quality_fit_operator_cue_issues),
            "validation_issues": quality_fit_operator_cue_issues,
            "triage_bucket_counts": count_by(quality_fit_operator_cue_rows, "triage_bucket"),
            "source_url_missing_rows": sum(1 for row in quality_fit_operator_cue_rows if row["source_url_missing"] == "yes"),
            "rights_class_missing_rows": sum(1 for row in quality_fit_operator_cue_rows if row["rights_class_missing"] == "yes"),
            "identity_metadata_missing_rows": sum(1 for row in quality_fit_operator_cue_rows if row["identity_metadata_missing"] == "yes"),
            "action_metadata_missing_rows": sum(1 for row in quality_fit_operator_cue_rows if row["action_metadata_missing"] == "yes"),
            "crop_metadata_missing_rows": sum(1 for row in quality_fit_operator_cue_rows if row["crop_metadata_missing"] == "yes"),
            "download_decision_review_eligible_rows": sum(1 for row in quality_fit_operator_cue_rows if row["download_decision_review_eligible"] == "yes"),
            "download_approval_recorded_rows": sum(1 for row in quality_fit_operator_cue_rows if row["download_approved"] == ("y" + "es")),
            "review_only_rows": sum(1 for row in quality_fit_operator_cue_rows if row["review_only"] == "true"),
            "publish_ready_rows": sum(1 for row in quality_fit_operator_cue_rows if row["publish_ready"] == "true"),
            "worksheet_csv": OUT_QUALITY_FIT_OPERATOR_CUE_CSV.as_posix(),
            "worksheet_md": OUT_QUALITY_FIT_OPERATOR_CUE_MD.as_posix(),
            "review_only": True,
            "asset_downloads": False,
            "headshot_writes": False,
            "approved_marker_writes": False,
    ...
    write_text(
        ...
        render_action_photo_download_decision_queue(download_decision_queue_rows, download_decision_queue_issues, generated_at),
    )
    write_json(
        OUT_DOWNLOAD_DECISION_QUEUE_JSON,
        {
            "version": VERSION,
            "status": "action_photo_download_decision_queue_ready" if not download_decision_queue_issues else "action_photo_download_decision_queue_has_validation_issues",
            "generated_at_utc": generated_at,
            "decision_rows": len(download_decision_queue_rows),
            "validation_issue_count": len(download_decision_queue_issues),
            "validation_issues": download_decision_queue_issues,
            "candidate_queue_ids": sorted({row["candidate_queue_id"] for row in download_decision_queue_rows}),
            "decision_priority_counts": {
                priority: sum(1 for row in download_decision_queue_rows if row["decision_priority"] == priority)
                for priority in sorted({row["decision_priority"] for row in download_decision_queue_rows})
            },
            "ready_for_human_download_decision_rows": sum(
                1 for row in download_decision_queue_rows if row["decision_priority"] == "P1_human_download_decision"
            ),
            "required_human_edits": DOWNLOAD_DECISION_REQUIRED_HUMAN_EDITS.split("|"),
            "allowed_download_destination": QUARANTINE_ROOT,
            "download_approved_yes_rows": sum(1 for row in download_decision_queue_rows if row["download_approved"] == "yes"),
            "blank_human_download_decision_rows": sum(1 for row in download_decision_queue_rows if not row["human_download_decision"]),
            "blank_manual_reviewer_rows": sum(1 for row in download_decision_queue_rows if not row["manual_reviewer"]),
            "blank_reviewed_at_utc_rows": sum(1 for row in download_decision_queue_rows if not row["reviewed_at_utc"]),
            "review_only_rows": sum(1 for row in download_decision_queue_rows if row["review_only"] == "true"),
            "publish_ready_rows": sum(1 for row in download_decision_queue_rows if row["publish_ready"] == "true"),
            "worksheet_csv": OUT_DOWNLOAD_DECISION_QUEUE_CSV.as_posix(),
            "worksheet_md": OUT_DOWNLOAD_DECISION_QUEUE_MD.as_posix(),
            "review_only": True,
            "asset_downloads": False,
            "headshot_writes": False,
            "approved_marker_writes": False,
    ...
    write_json(
        ...
        {
            "version": VERSION,
            "status": "action_photo_candidate_intake_ready" if not issues and not entity_source_issues and not womens_soccer_issues and not external_research_issues and not source_discovery_issues and not source_map_board_issues and not lead_return_schema_issues and not cutout_scoring_issues and not candidate_queue_issues and not manual_source_hunt_issues and not operator_worksheet_issues and not research_packet_issues and not research_return_issues and not research_return_paste_worksheet_issues and not research_run_bundle_issues and not quarantine_preflight_issues and not research_return_summary_issues and not quality_fit_issues and not quality_fit_operator_cue_issues and not download_decision_queue_issues and not wnba_hero_target_issues and not cutout_readiness_issues else "action_photo_candidate_intake_has_validation_issues",
            "generated_at_utc": generated_at,
            ...
            "action_photo_manual_source_hunt_board_rows": len(manual_source_hunt_rows),
            "action_photo_manual_source_hunt_board_validation_issue_count": len(manual_source_hunt_issues),
            ...
            "action_photo_research_return_paste_worksheet_rows": len(research_return_paste_worksheet_rows),
            "action_photo_research_return_paste_worksheet_validation_issue_count": len(research_return_paste_worksheet_issues),
            ...
            "action_photo_quality_fit_operator_cue_rows": len(quality_fit_operator_cue_rows),
            "action_photo_quality_fit_operator_cue_validation_issue_count": len(quality_fit_operator_cue_issues),
            ...
            "validation_issue_count": len(issues) + len(entity_source_issues) + len(womens_soccer_issues) + len(external_research_issues) + len(source_discovery_issues) + len(source_map_board_issues) + len(lead_return_schema_issues) + len(cutout_scoring_issues) + len(candidate_queue_issues) + len(manual_source_hunt_issues) + len(operator_worksheet_issues) + len(research_packet_issues) + len(research_return_issues) + len(research_return_paste_worksheet_issues) + len(research_run_bundle_issues) + len(quarantine_preflight_issues) + len(research_return_summary_issues) + len(quality_fit_issues) + len(quality_fit_operator_cue_issues) + len(download_decision_queue_issues) + len(wnba_hero_target_issues) + len(cutout_readiness_issues),
            ...
            "action_photo_manual_source_hunt_board_csv": OUT_MANUAL_SOURCE_HUNT_BOARD_CSV.as_posix(),
            "action_photo_manual_source_hunt_board_md": OUT_MANUAL_SOURCE_HUNT_BOARD_MD.as_posix(),
            "action_photo_manual_source_hunt_board_json": OUT_MANUAL_SOURCE_HUNT_BOARD_JSON.as_posix(),
            ...
            "action_photo_research_return_paste_worksheet_csv": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_CSV.as_posix(),
            "action_photo_research_return_paste_worksheet_md": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_MD.as_posix(),
            "action_photo_research_return_paste_worksheet_json": OUT_RESEARCH_RETURN_PASTE_WORKSHEET_JSON.as_posix(),
            ...
            "action_photo_quality_fit_operator_cue_csv": OUT_QUALITY_FIT_OPERATOR_CUE_CSV.as_posix(),
            "action_photo_quality_fit_operator_cue_md": OUT_QUALITY_FIT_OPERATOR_CUE_MD.as_posix(),
            "action_photo_quality_fit_operator_cue_json": OUT_QUALITY_FIT_OPERATOR_CUE_JSON.as_posix(),
