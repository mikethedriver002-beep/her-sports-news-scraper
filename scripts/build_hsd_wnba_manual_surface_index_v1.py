...
SURFACES = [
    ...
    SurfaceSpec(
        ...
        surface_name="WNBA Aces scout and deck",
        packet_root_name="wnba_official_team_source_scout_v1",
        primary_artifact_name="wnba_aces_source_scout_report.md",
        report_name="wnba_aces_source_scout_report.md",
        csv_name="wnba_aces_source_scout_board.csv",
        priority="P4_aces_manual_source_check",
        surface_type="metadata_scout",
        manual_action="Inspect WAFS001/WAFS003/WAFS002 first; carry forward only if the OG image reads as true 4:5 action material.",
        attachment_guidance="Attach report, board CSV, intake CSV, and deck HTML when emailing Mike.",
        blunt_read="Strong official Aces metadata scout with five clear remote image leads; still needs manual visual confirmation.",
    ),
    SurfaceSpec(
        ...
        priority="P5_storm_source_context",
        surface_type="metadata_scout",
    ...
    SurfaceSpec(
        ...
        priority="P6_fever_source_context",
        surface_type="metadata_scout",
    ...
    SurfaceSpec(
        surface_id="wnba_fire_source_scout",
        surface_name="WNBA Fire gallery source scout",
        packet_root_name="wnba_official_source_expansion_highres_v12",
        primary_artifact_name="wnba_fire_gallery_source_scout_report.md",
        report_name="wnba_fire_gallery_source_scout_report.md",
        csv_name="wnba_fire_gallery_source_scout_board.csv",
        priority="P7_fire_gallery_context",
        surface_type="metadata_scout",
        manual_action="Open the Fire gallery scout, then inspect the high-res gallery frames before any downstream review decisions.",
        attachment_guidance="Attach the report, board CSV, intake CSV, and deck HTML when sharing this lane.",
        blunt_read="Useful Portland Fire gallery scout with real high-res photo-frame leads and one branded opener tile.",
    ),
    SurfaceSpec(
        ...
        priority="P7_visual_hold_unless_selected",
...
def build_report(manifest: dict[str, Any], rows: list[dict[str, str]]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Blunt Read",
            "",
            "- Use the Fever visual-rank board first, then the Storm visual-rank board, then the Aces source scout.",
