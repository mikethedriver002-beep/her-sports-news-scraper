...
def score_asset(team: Dict[str, str], path: Path) -> int:
    if not identity_match(team, path):
        return -999
    low = path.as_posix().lower()
    score = 100
    if "logo" in low:
...
def main() -> None:
    ...
    for team in teams:
        ...
        if logo_path:
            verified += 1
            logo_rows.append({"team_id": team_id, "asset_type": "primary_logo", "file_path": logo_path.as_posix(), "file_exists": "true", "approved": "true", "required": "true", "last_verified_utc": now_iso(), "source_note": "strict_exact_identity_match"})
        else:
            recommended = f"assets/leagues/wnba/teams/{team_id}/logo.png"
            logo_rows.append({"team_id": team_id, "asset_type": "primary_logo", "file_path": recommended, "file_exists": "false", "approved": "false", "required": "true", "last_verified_utc": now_iso(), "source_note": "missing_required_exact_logo"})
            missing_rows.append({"team_id": team_id, "team_name": team_name, "required_asset": "primary_logo", "reason": "required exact team logo file not found", "recommended_path": recommended})
    ...
    summary = {"version": "hsd-wnba-asset-registry-v1.1-strict", "generated_at_utc": now_iso(), "teams": len(teams), "team_logos_verified": verified, "missing_team_logos": len(missing_rows), "team_logo_policy": "required_exact_identity_match", "scan_roots": [p.as_posix() for p in SCAN_ROOTS]}
    SUMMARY_JSON.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    lines = ["# HSD WNBA Asset Registry v1.1", "", f"Generated: {summary['generated_at_utc']}", "", "## Counts", "", f"- teams: {summary['teams']}", f"- team logos verified: {summary['team_logos_verified']}", f"- missing required team logos: {summary['missing_team_logos']}", "", "## Missing team logos", ""]
