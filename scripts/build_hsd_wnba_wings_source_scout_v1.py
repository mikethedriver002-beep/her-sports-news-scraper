...
VERSION = "hsd-wnba-wings-source-scout-v1-review-only"
GENERATED_BY = "scripts/build_hsd_wnba_wings_source_scout_v1.py"
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "tmp" / "wnba_official_source_expansion_highres_v7"
DEFAULT_LATEST_OUTPUT_DIR = REPO_ROOT / "outputs" / "local" / "latest" / "files" / "wnba_official_source_expansion_highres_v7"
...
TINY_THUMBNAIL_RE = re.compile(r"-(?:185x148|260x190|300x78|320x180|594x594|640x360|1024x576)(?:-\d+)?(?=\.)", re.I)
...
def is_useful_wings_image_url(url: str) -> bool:
    parsed = urlparse(clean(url))
    lowered = parsed.geturl().lower()
    if parsed.netloc != "cdn.wnba.com":
        return False
    if "/sites/1611661321/" not in parsed.path:
        return False
    ...
    if any(token in lowered for token in ("favicon", "apple-touch-icon", "/static/next/", "/headshots/")):
        return False
    return any(token in lowered for token in ("gettyimages-", "-scaled"))
...
def extract_image_urls(page_url: str, text: str, parser: PageParser) -> list[str]:
    ...
    for raw in candidates:
        ...
        if candidate in seen:
            continue
        seen.add(candidate)
        if is_useful_wings_image_url(candidate):
...
def score_row(title: str, description: str, candidate_url: str, paywall_hit: str) -> tuple[int, str, list[str]]:
    score = 70
    flags: list[str] = ["matchup_level_identity_only"]
    ...
    if "wings" in combined or "wings" in url or "dallas" in combined:
        score += 8
    if any(term in combined for term in ("recap", "game recap", "win", "defeat", "overtime")):
        score += 8
    if "gettyimages-" in url or "-scaled" in url:
        score += 8
...
def source_family_rows(
    ...
    *,
    ...
    for index, seed in enumerate(seed_rows, start=1):
        ...
        candidate_id = f"DWGS{index:03d}"
        evidence_summary = clean(parsed["description"] or parsed["title"] or "Official Dallas Wings recap page.")
        notes = (
        ...
        intake_rows.append(
            {
                ...
                "identity_anchor_url": clean(seed.get("identity_anchor_url") or "https://wings.wnba.com/roster"),
                ...
                "manual_next_action": "Open the Wings recap, confirm the selected high-resolution CDN image still reads as a clean Dallas Wings action lead, and carry forward only if the 4:5 crop works for review-only intake.",
        ...
        board_rows.append(
            {
                ...
                "source_family_id": "wnba_wings_official_game_recaps",
                "candidate_queue_id": candidate_id,
                "seed_id": seed_id,
                "entity_id": entity_id,
...
def render_report(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        "# WNBA Wings Official Game Recap Source Scout V1",
        "",
        f"Generated: `{manifest['generated_at_utc']}`",
        "",
        "Review-only metadata-first source scout for the official Dallas Wings recap lane.",
        "",
        "## Summary",
        "",
...
def build_packet(
    *,
    ...
    scout_seed_path = output_dir / "wnba_wings_source_scout_seed.csv"
    intake_path = output_dir / "wnba_wings_source_scout_intake.csv"
    board_path = output_dir / "wnba_wings_source_scout_board.csv"
    report_path = output_dir / "wnba_wings_source_scout_report.md"
    manifest_path = output_dir / "manifest.json"
    deck_output_dir = output_dir / "review_deck"

    write_csv(scout_seed_path, seed_rows, list(seed_rows[0].keys()) if seed_rows else [])
    ...
    manifest: dict[str, Any] = {
        ...
        "status": "wnba_wings_source_scout_ready" if intake_rows else "wnba_wings_source_scout_empty",
        "review_only": True,
        ...
        "source_family_usefulness_verdict": "useful_high_res_official_recap_family" if board_rows else "not_useful_no_candidate_rows",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the Dallas Wings official recap source scout packet.")
