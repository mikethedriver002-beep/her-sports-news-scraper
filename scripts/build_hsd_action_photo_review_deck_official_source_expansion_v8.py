...
VERSION = "hsd-action-photo-review-deck-official-source-expansion-v8-review-only"
DEFAULT_OFFICIAL_EXPANSION_CSV = Path(
    "outputs/local/tmp/action_photo_official_source_expansion_v8/action_photo_candidate_intake.csv"
)
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/action_photo_review_deck_official_source_expansion_v8")
DEFAULT_LIMIT = 24


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a local review-only action-photo deck from WTA official-source expansion candidates."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    manifest["source_packet"] = "action_photo_official_source_expansion_v8"
