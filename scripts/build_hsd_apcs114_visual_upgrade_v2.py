...
VERSION = "hsd-apcs114-visual-upgrade-v2-review-only"
GENERATED_BY = "scripts/build_hsd_apcs114_visual_upgrade_v2.py"
...
DEFAULT_OUTPUT_DIR = Path("outputs/local/tmp/apcs114_visual_upgrade_v2")
RUNNER_NAME = "apcs114_visual_upgrade_runner.py"
SPECS_NAME = "apcs114_visual_upgrade_specs.json"
...
BURN_IN = "REVIEW ONLY - APCS114 VISUAL UPGRADE"
...
def build_proof_specs() -> list[dict[str, Any]]:
    return [
        {
            "proof_id": "upgrade_01_darkroom_score_anchor",
            "proof_name": "Darkroom Score Anchor",
            "filename": "upgrade_01_darkroom_score_anchor.png",
            "texture_filename": "upgrade_01_darkroom_score_anchor_texture.png",
            "crop_strategy": "wider_center_right_4x5_subject_context",
            "crop_center": [0.655, 0.535],
            "crop_zoom": 1.08,
            "texture_grade": {
                "brightness": 0.74,
                "contrast": 1.22,
            ...
            "visual_strength": "strongest_controlled_score_candidate",
            "known_limit": "group_celebration_source_not_solo_hero",
            "accent_color": [229, 176, 75],
            "background_top": [5, 7, 12],
            "background_bottom": [12, 18, 28],
            "headline": "FINAL",
            "subhead": "THORNS FIND THE EDGE",
            "score": "2 - 1",
            "caption": "APCS114 / SOPHIA WILSON",
            "photo_location": [-1.36, 0.48, 0.20],
            "photo_scale": [1.62, 1.0, 2.64],
            "photo_rotation_z": -1.0,
            "text_anchor_x": 1.02,
        },
        {
            "proof_id": "upgrade_02_field_depth_story",
            "proof_name": "Field Depth Story",
            "filename": "upgrade_02_field_depth_story.png",
            "texture_filename": "upgrade_02_field_depth_story_texture.png",
            "crop_strategy": "wide_group_celebration_4x5_story_context",
            "crop_center": [0.64, 0.535],
            "crop_zoom": 1.0,
            "texture_grade": {
                "brightness": 0.70,
                "contrast": 1.30,
            ...
            "visual_strength": "best_contextual_news_candidate",
            "known_limit": "wide_group_frame_needs_manual_subject_check",
            "accent_color": [226, 93, 82],
            "background_top": [7, 10, 16],
            "background_bottom": [22, 20, 31],
            "headline": "LATE GOAL",
            "subhead": "CELEBRATION HOLDS",
            "score": "MATCHWEEK 5",
            "caption": "REVIEW-ONLY SOURCE READ",
            "photo_location": [1.34, 0.58, 0.02],
            "photo_scale": [1.46, 1.0, 2.76],
            "photo_rotation_z": 0.8,
            "text_anchor_x": -2.62,
        },
        {
            "proof_id": "upgrade_03_clean_subject_read",
            "proof_name": "Clean Subject Read",
            "filename": "upgrade_03_clean_subject_read.png",
            "texture_filename": "upgrade_03_clean_subject_read_texture.png",
            "crop_strategy": "right_subject_4x5_magazine_read",
            "crop_center": [0.705, 0.535],
            "crop_zoom": 1.16,
            "texture_grade": {
                "brightness": 0.76,
                "contrast": 1.24,
            ...
            "composition_treatment_mode": "layered_news_card_photo_plane",
            "visual_strength": "cleanest_subject_read_candidate",
            "known_limit": "teammate_overlap_remains_visible",
            "accent_color": [214, 198, 145],
            "background_top": [10, 13, 20],
            "background_bottom": [16, 20, 28],
            "headline": "SOURCE READ",
            "subhead": "SOPHIA WILSON",
            "score": "APCS114",
            "caption": "NO APPROVAL / REVIEW ONLY",
            "photo_location": [1.30, 0.50, 0.04],
            "photo_scale": [1.28, 1.0, 2.42],
...
def apply_editorial_grade(image: Any, grade: dict[str, Any] | None) -> Any:
    if ImageEnhance is None or ImageOps is None or Image is None:
        raise RuntimeError("Pillow image enhancement support is required for APCS114 visual upgrade textures")
    grade = grade or {}
    ...
    graded = ImageEnhance.Color(graded).enhance(float(grade.get("color", 0.92)))
    graded = ImageEnhance.Sharpness(graded).enhance(float(grade.get("sharpness", 1.06)))
    ...
    if top_scrim > 0:
        ...
        alpha_pixels = []
        fade_height = max(1, int(height * 0.38))
        for y in range(height):
            local = max(0.0, 1.0 - (y / fade_height))
            value = int(255 * top_scrim * local)
            alpha_pixels.extend([value] * width)
...
def crop_texture(
    ...
    with Image.open(source_image) as source:
        ...
        fitted = apply_editorial_grade(fitted, grade)
...
def build_runner_script() -> str:
    return textwrap.dedent(
        ...
        def add_background(spec: dict[str, object]) -> None:
            ...
            low_glow = (accent[0] * 0.16, accent[1] * 0.16, accent[2] * 0.16, 1.0)
            ...
            add_plane("AccentGlow", (-2.05, 2.45, 1.2), (1.35, 1.0, 1.45), material("AccentGlowMat", low_glow, 0.55, 0.16))
        ...
        def add_burn_in() -> None:
            ...
            add_text("BurnIn", "REVIEW ONLY - APCS114 VISUAL UPGRADE", (0.0, 0.12, -3.38), 0.135, (0.92, 0.94, 0.98, 1.0), True, "CENTER", 0.05)
        ...
        def render_news_depth(spec: dict[str, object]) -> None:
            accent = tuple(float(v) / 255.0 for v in spec["accent_color"]) + (1.0,)
            x = float(spec["text_anchor_x"])
            add_panel("NewsTextRail", (-2.05, 0.60, 0.78), (1.00, 0.045, 1.35), (0.018, 0.022, 0.030, 1.0), 0.0)
            ...
            add_text("Headline", str(spec["headline"]), (x, 0.08, 1.50), 0.34, (0.96, 0.97, 0.99, 1.0), True, "LEFT")
            add_text("Subhead", str(spec["subhead"]), (x, 0.08, 1.08), 0.18, accent, True, "LEFT")
            add_text("Score", str(spec["score"]), (x, 0.08, 0.40), 0.27, (0.82, 0.86, 0.93, 1.0), True, "LEFT")
...
def create_contact_sheet(output_dir: Path, specs: list[dict[str, Any]]) -> Path:
    ...
    draw.text((34, 24), "APCS114 REVIEW-ONLY VISUAL UPGRADE V2", fill=(244, 246, 250), font=font)
    draw.text((34, 55), "Controlled exposure, wider crops, no asset approval, no publish-ready state.", fill=(190, 198, 210), font=small)
...
def build_report(manifest: dict[str, Any]) -> str:
    ...
    return f"""# APCS114 Visual Upgrade V2

Status: `{manifest['status']}`
Version: `{VERSION}`

This packet pushes the APCS114 Sophia Wilson review-only quarantine candidate beyond the first renderer proof by controlling source exposure, using wider 4:5 crops, and testing more deliberate Blender-backed editorial staging. It is not asset approval, renderer approval, publish-ready output, or publishing.

## Visual Read

- Strongest proof: `upgrade_01_darkroom_score_anchor` because it gives the clearest score/social composition while reducing the washed-out source treatment from v1.
- Useful alternate: `upgrade_02_field_depth_story` because it treats the full group frame as context instead of over-forcing a solo crop.
- Clean baseline: `upgrade_03_clean_subject_read` because it tests the tightest readable right-side subject crop while keeping text on a protected plane.
- Known limit: APCS114 is still a group celebration frame. V2 can improve the renderer read, but it cannot turn this into a clean solo player hero.
...
def build_packet(*, source_image: Path, output_dir: Path, blender_executable: Path | None, head_commit: str = "") -> dict[str, Any]:
    ...
    if render_result.returncode != 0 or missing_outputs or traceback_present:
        status = "apcs114_visual_upgrade_blocked_render_failed"
        contact_sheet_path = ""
    else:
        status = "apcs114_visual_upgrade_ready"
    ...
    manifest: dict[str, Any] = {
        ...
        "strongest_proof_id": "upgrade_01_darkroom_score_anchor",
        "review_only": True,
        "approved_marker_writes": False,
        **FALSE_GUARDRAILS,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build APCS114 review-only Blender visual upgrade packet.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if manifest["status"] == "apcs114_visual_upgrade_ready" else 1
