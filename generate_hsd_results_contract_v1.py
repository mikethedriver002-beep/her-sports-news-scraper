...
def parse_dt_value(v: str) -> datetime | None:
    s = clean(v)
    if not s:
        return None
    try:
...
def parse_date(row: Dict[str, str]) -> str:
    for k in ["event_date", "result_date", "scheduled_date_local", "game_date", "date", "event_date_local", "scheduled_datetime_local"]:
        d = parse_date_value(row.get(k, ""))
        if d:
            return d
    for k in ["scheduled_start_utc", "event_datetime", "completed_at", "completed_at_utc", "start_time", "status"]:
        d = parse_date_value(row.get(k, ""))
        if d:
            return d
...
def reference_dt(row: Dict[str, str], event_date: str) -> datetime | None:
    for key in ["completed_at_utc", "completed_at", "scheduled_start_utc", "scheduled_datetime_utc", "event_datetime", "scheduled_datetime_local", "start_time"]:
        dt = parse_dt_value(row.get(key, ""))
        if dt:
            return dt
    return date_to_dt(event_date)
...
def eligibility_for(kind: str, event_date: str, row: Dict[str, str]) -> Tuple[str, str, str, float | str]:
    if not event_date:
        return "blocked", "missing_event_date", "Yes", ""
    dt = reference_dt(row, event_date)
    if not dt:
...
def score_value(row: Dict[str, str], *keys: str) -> str:
    for key in keys:
...
def normalize(row: Dict[str, str], source_file: str, run_id: str) -> Dict[str, str]:
    ...
    eligibility, reason, review, age = eligibility_for(kind, e_date, row)
