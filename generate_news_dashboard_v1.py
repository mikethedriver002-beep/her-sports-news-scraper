...
def card_grid(rows: List[Dict[str, str]], empty: str = "Nothing to show.") -> str:
    ...
    for row in rows:
        ...
        <div class="card">
          ...
          <div class="small"><b>Source confidence:</b> {esc(row.get('source_publish_grade') or row.get('source_confidence_tier'))} | {esc(row.get('source_confidence_score'))}/100</div>
          <div class="small"><b>Source reason:</b> {esc(row.get('source_confidence_reason'))}</div>
...
<title>Her Sports Daily News Sync v1.8</title>
<style>
:root {{
...
<body>
<header><div class="wrap brand"><div class="bug">HER<br>SPORTS<br>DAILY</div><div><h1>News Sync v1.8</h1><div class="sub">Generated {esc(datetime.now(timezone.utc).isoformat())}. Source-backed news packets on top of Results Desk. v1.8 makes source matching sport-strict and removes false source-failure noise.</div></div></div></header>
<main>
