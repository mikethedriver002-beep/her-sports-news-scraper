# Her Sports Daily Daily Content Hub

Generated: 2026-06-09T22:02:12.504576+00:00

## What this package includes

- `daily_content_command_center.csv`
- `tonight_in_the_w_package.csv`
- `must_post_carousels.csv`
- `story_poll_package.csv`
- `caption_bank_v2.csv`
- `reel_script_package.md`
- `graphics_copy_package.md`

## Must Post

### Rank 1: Caitlin Clark Hits Game-Winning 3-Pointer to Push Fever Past Mystics

- Sport: WNBA
- Story type: Game Recap / Result
- Content family: Tonight in the W
- Recommended asset: 4-slide matchup/pregame graphic
- Hook: Here is the quick version of what happened and why it matters.
- Caption starter: Caitlin Clark Hits Game-Winning 3-Pointer to Push Fever Past Mystics Here is the quick breakdown and why it matters.
- Visual brief: Use a quick recap carousel with final result, top performer, and next game.

### Rank 2: A’ja Wilson Makes WNBA History in Aces vs. Storm Win

- Sport: WNBA
- Story type: Record / Milestone
- Content family: Breaking / Must Post Carousel
- Recommended asset: 4-slide news carousel
- Hook: This is the kind of milestone that shows where the game is headed.
- Caption starter: A’ja Wilson Makes WNBA History in Aces vs. Storm Win What should we cover next?
- Visual brief: Use a milestone carousel with the record, context, and why it matters.

## Maybe Post

### Rank 3: Texas’s 2026 WCWS Finals Win Shatters NCAA Softball Viewership Records

- Sport: Softball
- Story type: Game Recap / Result
- Content family: Feed Post
- Recommended asset: single recap card or quick carousel
- Hook: Here is the quick version of what happened and why it matters.
- Caption starter: Texas’s 2026 WCWS Finals Win Shatters NCAA Softball Viewership Records Here is the quick breakdown and why it matters.
- Visual brief: Use a quick recap carousel with final result, top performer, and next game.

### Rank 5: Nelly Korda continues her historic march with 2026 U.S. Women's Open title

- Sport: Golf / LPGA
- Story type: Record / Milestone
- Content family: Context Carousel
- Recommended asset: 3 to 4-slide milestone graphic
- Hook: This is the kind of milestone that shows where the game is headed.
- Caption starter: Nelly Korda continues her historic march with 2026 U.S. Women's Open title What should we cover next?
- Visual brief: Use a milestone carousel with the record, context, and why it matters.

### Rank 6: Hilary Knight will play for PWHL expansion Detroit via sign-and-trade with Las Vegas, AP source says

- Sport: PWHL / Women's Hockey
- Story type: League Expansion
- Content family: Context Carousel
- Recommended asset: single feed card
- Hook: Another market is betting big on women's sports.
- Caption starter: Hilary Knight will play for PWHL expansion Detroit via sign-and-trade with Las Vegas, AP source says More teams, more investment, more proof that the demand is real.
- Visual brief: Use a map, team/logo style graphic, or market comparison carousel.

## Save for Weekend

### Rank 7: Sol Ruca Defeats Lyra Valkyria, Wins First WWE Women's IC Title Defense On Raw

- Sport: Women's Sports
- Story type: Game Recap / Result
- Content family: Feed Post
- Recommended asset: single recap card or quick carousel
- Hook: Here is the quick version of what happened and why it matters.
- Caption starter: Sol Ruca Defeats Lyra Valkyria, Wins First WWE Women's IC Title Defense On Raw Here is the quick breakdown and why it matters.
- Visual brief: Use a quick recap carousel with final result, top performer, and next game.

### Rank 8: Athletes Unlimited and TOGETHXR Debut “Everyone Watches Women’s Softball” Amid Historic Growth for the Game

- Sport: Softball
- Story type: Record / Milestone
- Content family: Context Carousel
- Recommended asset: 3 to 4-slide milestone graphic
- Hook: This is the kind of milestone that shows where the game is headed.
- Caption starter: Athletes Unlimited and TOGETHXR Debut “Everyone Watches Women’s Softball” Amid Historic Growth for the Game What should we cover next?
- Visual brief: Use a milestone carousel with the record, context, and why it matters.

### Rank 9: Toronto Sceptres Pushing Through Expansion Process

- Sport: PWHL / Women's Hockey
- Story type: League Expansion
- Content family: Context Carousel
- Recommended asset: single feed card
- Hook: Another market is betting big on women's sports.
- Caption starter: Toronto Sceptres Pushing Through Expansion Process More teams, more investment, more proof that the demand is real.
- Visual brief: Use a map, team/logo style graphic, or market comparison carousel.

### Rank 10: Michigan Women’s Basketball announces Big Ten home/away opponents

- Sport: NCAA Women's Basketball
- Story type: Breaking / Transaction
- Content family: Feed Post
- Recommended asset: single feed card
- Hook: Michigan Women’s Basketball announces Big Ten home/away opponents
- Caption starter: Michigan Women’s Basketball announces Big Ten home/away opponents What should we cover next?
- Visual brief: Use a simple NCAA Women's Basketball news card with headline, context, and one engagement question.
